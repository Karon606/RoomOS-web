...
- **백로그(운영자, 2026-08-18)**: 환경설정 정보구조 재정리 — 기능 메뉴 위치·세팅 카테고리 전면 재검토. 홈의 "소개 페이지에서 내릴 수 있는 방" 기능도 환경설정의 웹사이트 설정 계열 아래로 이동 검토. 착수 시 IA 패널(UX·웹디자이너·도메인)로 재배치안을 만들어 운영자 승인 후 시공.
