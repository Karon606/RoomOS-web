...
- **상태 변경 UX**: 거주중→퇴실예정 전환이 '수정' 진입 후 드롭다운이라 어색 → **명시적 전환 액션 채택**
  (2026-05-23 결정): 상세에 현재 상태기준 행동 버튼('퇴실 예정 처리' 등) → 필요한 칸만 떠서 적용. ②와 묶어 설계.
- **동적 입력 배치**: 고객 정보 입력/수정 시 상태에 따라 관련 필드를 인접 배치 (퇴실예정→퇴실예정일,
  예약→문의일시·예약일·입주희망일, 투어대기→투어일정, 이름→연락처). 고정 X, 상태/상황 따라 유동.
