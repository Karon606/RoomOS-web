...
- **월별 히스토리 끊기**: 타임라인이 길어짐 → 상단 표시 월 기준으로 그 달 내역만 표시, 월 넘어가면
  전월 최종내역이 매월 1일자 '이월분'으로 carry-over (예: 쌀 4/27 마지막구매 → 5/1에 4월말 최종이 이월).
  단 월 컷오프는 '수령 확정' 기준 (4월 구매·미수령 건은 5월로 넘어옴). 지출 관련 표시는 구매 기준.
- **임시저장 cross-mode 반영**: 아이템별로 임시저장한 수량이 위치별 점검에서 안 보여 혼란 →
  두 모드가 같은 드래프트를 공유/반영하도록 개선 (현재 item=locationId null, location=locId 로 분리됨).
