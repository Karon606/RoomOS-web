...
### 진행 중 — 병합 확인 기능 (feat/merge-confirm 브랜치, 2026-05-23)
- 영수증→재고 자동등록 시 라벨 변형 카드 난립 → 별칭+유사라벨 후보 제시, 항상 확인 후 병합.
- ✅ 백엔드 완료(브랜치): TrackedItemMergeRule(LINK/MUTE) + migrate_tracked_item_merge_rules.sql
  + mergeTrackedItems 별칭 기록 + seed 후보 수집(정규화 비교) + applyMergeDecision/getMergeRules/deleteMergeRule
- ⏳ 남음: 확인 모달 + 규칙 관리 모달(거절 되돌리기) + 헤더 버튼, 마이그레이션 적용, 배포
- 상세: memory project_merge_confirm

### 재고관리 개선 (2026-05-23 요청)
- ✅ **월별 히스토리 끊기** (2026-05-23 완료, main 843f793): 상세 타임라인을 전역 월(?month=) 기준
  그 달만 표시. 월 컷오프 — 점검·무상입수=날짜, 구매=수령확정(receivedAt)(미수령은 현재 월로 이월).
  매월 1일자 '이월분' 줄(전월말 마지막 점검 잔량) + 타임라인 탭 월 이전/다음 네비.
  · 알려진 한계: 이월분은 '마지막 점검 잔량' 기준(점검 후 입수분은 다음 점검 때 반영).
- ✅ **임시저장 cross-mode 반영** (2026-05-23 완료, main 835c2b7): 아이템별↔위치별 드래프트 공유.
  getLocationDrafts가 null-draft의 해당 위치 값 병합, CheckForm이 위치별 드래프트 cross-merge.

### 고객/수납/호실 UX 개선 (2026-05-23 요청)
- ✅ **상태 변경 UX** (2026-05-23 완료, main 00afefd): 상세 모달 상단에 상태별 '다음 단계' 전환 버튼
  + 필요 값만 묻는 미니폼 → applyStatusTransition (호실·이력 자동, 퇴실 시 보증금 환불 동시). transitionsFor 로 전환맵 정의.
- ✅ **동적 입력 배치** (2026-05-23 완료): 연락처를 기본정보 뒤로(이름→연락처 인접), 상태별 단계정보
  (문의일시·투어예정일·예약확정)를 상태 선택 바로 아래로 클러스터링.
  · 남은 개선 여지: 입주희망일은 아직 청소비 행에 있음(상태 클러스터로 이동은 추후 검토).
- **3개 페이지 상세 팝업 통합 네비** (풀 통합 결정, 단계적 구현):
  · ✅ 1단계 (2026-05-23, main 56bfbda): 전역 공유 모달 components/entity-modal/EntityModal.tsx
