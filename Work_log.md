...
- **검증**: tsc 0 · 회귀 40 신설 · verify:fast exit 0(커밋 훅 6회) · **verify:db 기지 예외(서류 표시값 소재지 3건)에서만 정지**, 캘린더 축 행 41·막대 72 위반 0 · 청소 고아 0 · 프로덕션 빌드 exit 0 · eslint 491 유지(신규 0) · **폼 2 x 상태 2 x 폭 320/360/390 x 라이트·다크 24조합에서 문서 넘침 0 · 칸 밖 넘침 0 · 글자 넘침 0**(프로덕션 CSS 로 모달 본문 재현) · 칸 높이 89~106px · 입력 높이 42px(형제 칸과 동일) · 보조줄 대비 라이트 5.12 다크 4.91.
