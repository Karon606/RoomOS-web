...
- **고객관리 진입 팝업**: 퇴실예정처리/퇴실일변경(신규 전환)으로 expectedMoveOut 입력 시 정산은 자동 적용 안 함. 단 `shouldOfferCheckoutProration`(일할 부분기간 + 퇴실일 오늘 기준 31일 이내)이면 '퇴실 정산?' 팝업 → 예: `entityModal.open({openCheckoutProration})`로 수납 모달 full+위젯 자동펼침·날짜 프리필·미리보기 / 아니오: 날짜만. (Seed→PrismShellView→PaymentBody→Widget autoOpen 스루) 늦은정산·선납환불 둘 다 커버.
