...
- **범위 밖 발견(기록만)**: `ensureOpenStay` 자가 치유 경로 5곳은 여전히 이사일이 '오늘'로 박힌다 · `lib/overlapAck` 의 겹침 구간이 계약 날짜라 이사한 계약의 **옛 방** 겹침은 [겹침 확인]으로 못 덮는다(안전한 쪽 실패) · `check-room-availability-drift` 축 ② 도 같은 이유로 옛 방 겹침을 못 본다 · **이사로 비는 옛 방에 퇴실 청소가 안 잡힌다** · '오늘로' 버튼이 §10 primary 색 전환 대신 opacity · ConflictBtn 의 busy 가 §10 스피너 규정 미충족 · '오늘' 칩이 범위 마지막 날이면 트랙 밖으로 잘림 · RoomStay 에 propertyId 인덱스 없음(다점포 전환 시 검토).
