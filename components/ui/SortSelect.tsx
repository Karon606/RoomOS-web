...
// 두 타깃 모두 44px(§09 터치 타겟 · §12 --input-h-touch). 토글은 select 와 세로로 맞물리므로
// 한쪽만 줄이면 정렬이 깨진다 — 높이를 바꾸려면 둘을 같이 바꾼다.
    ...
    <div className={`inline-flex items-center gap-1.5 ${className ?? ''}`}>
      ...
      <div className="relative">
        <select
          ...
          className="appearance-none h-[var(--input-h-touch)] bg-[var(--cream)] border border-[var(--warm-border)] text-[var(--warm-dark)] text-xs font-medium rounded-sm pl-3 pr-7 outline-none focus:border-[var(--coral)] transition-colors"
        >
          {options.map(o => (
      ...
      <button
        ...
        className="flex h-11 w-11 items-center justify-center rounded-lg border border-[var(--warm-border)] bg-[var(--cream)] text-xs text-[var(--warm-mid)] transition-colors hover:border-[var(--coral)] hover:text-[var(--warm-dark)]"
      >
