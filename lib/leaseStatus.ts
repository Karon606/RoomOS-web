 ...
 *
 * 사람 축도 이 순서를 쓴다(2026-08-13) — 고객 수납 모달의 계약 전환 세그먼트다. 묻는 대상만 다르고
 * 층 위계·동률 규칙은 같으며, 첫 원소가 primaryTenantLease 와 일치한다(거주 계약이 하나 이하인
 * 정상 범위에서 — test-lease-primary 가 그 항등을 잠근다). primaryRoomLease/primaryTenantLease 처럼
 * 순서 문장을 두 벌 두지 않기 위해 여기 하나를 그대로 부른다.
 *
