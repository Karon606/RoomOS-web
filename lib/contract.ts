...
export type ContractTemplate = {
  ...
  oathText: string      // "위 계약 내용과 생활 수칙을 모두 읽고 이해하였으며 ..."
}
...
export const DEFAULT_CONTRACT_TEMPLATE: ContractTemplate = {
  ...
  sections: [
    {
      ...
      items: [
        ...
        '- 기본 계약기간은 1개월로 합니다. (이용료는 별도될 수 있습니다.)',
        '- 이용료는 매월 선납을 원칙으로 합니다. (반드시 입실일 기준 전일까지 납부해야 합니다.)',
        '- 1개월 미만으로 연장할 경우 일 단위 결제하며 1일당 30,000원 입니다.',
        '- 이용료 납부일 3일 경과 후에도 별 다른 협의 없이 미납할 경우 방을 회수하고 방안의 짐은 창고에 보관하며 2주 이내 찾아가지 않을 경우 임의로 폐기합니다.',
    ...
    },
    {
      ...
      items: [
        ...
        '- 퇴실 시 이용료 환불은 관계 법령 및 공정거래위원회 기준에 따릅니다.{{환불규정}}{{청소비공제}}',
  ...
  // 종전 문안("어떠한 조치에도 이의를 제기하지 않을 것에 대해 서약합니다")은 고객의 항변권을
  // 통째로 배제해 약관규제법상 무효 소지였다(법률 패널 2026-09-09). 퇴실 권한은 본문 4절 열거
  // 사유와 민법에서 나오므로 이 문장이 없어도 사업자가 잃는 것이 없다. 되돌리지 마라 —
  // scripts/check-contract-unfair-clause.ts 가 옛 문안의 복귀를 잡는다.
  oathText: '위 계약 내용과 생활 수칙을 모두 읽고 이해하였으며, 위반 시 본 계약서에 정한 바에 따라 계약 해지 등의 조치가 있을 수 있음을 확인하고 서명합니다.',
}
...
// **기준액은 잔여 이용금액이다**(2026-08-29 정정). 종전 문구는 총 결제금액의 10% 였는데, 고시는
// 개시일 이후 해지에 대해 잔여이용금액의 10% 를 말한다 — 총 결제금액의 10% 는 개시일 **이전**
// 해지, 즉 하루도 안 산 경우의 기준이다.
export function buildRefundClause(): string {
  return '중도 퇴실 시 환불액은 「총 결제금액 − (1일 이용요금 × 실제 이용일수) − 위약금(잔여 이용금액의 10%)」으로 산정하며, 1일 이용요금은 월 이용료의 30분의 1로 하고, 잔여 이용금액은 총 결제금액에서 실제 이용일수분을 뺀 금액으로 합니다.'
}

// ── 추가 호실 특약(보관 용도) ────────────────────────────────────────
//
// 창고·사무실처럼 거주용이 아닌 방이 추가 호실로 딸린 계약서에만 코드가 붙이는 절이다.
...
export const DEFAULT_SUB_LEASE_ADDENDUM: SubLeaseAddendum = {
  ...
  items: [
    ...
    '추가 호실에는 화기(불씨를 일으키는 기기·화염 기구) 및 인화성/폭발성/휘발성 물질, 부패하거나 악취를 유발하는 물품, 동식물, 법령상 보관이 금지된 물품을 보관할 수 없습니다.',
    ...
    '추가 이용료는 본 계약 이용료와 같은 방법으로 매월 선납하며, 환불과 미납 시 처리, 연락두절 시 처리는 본 계약서의 해당 조항을 추가 호실에 동일하게 적용합니다.',
...
export const DEFAULT_SHORT_STAY_ADDENDUM: SubLeaseAddendum = {
  ...
  items: [
    ...
    '이용료는 단기 입실 요금표에 따라 산정합니다. {{단기요금표}}',
    '계약 기간이 1개월에 이르는 경우 1개월 이용료를 상한으로 합니다.',
    '계약한 기간을 채우지 못하고 퇴실하는 경우에도 계약한 기간의 이용료를 부담합니다.',
  ],
}
...
export const DEFAULT_DISPOSAL_CONSENT: DisposalConsentTemplate = {
  ...
  body:
    "본인(입실자)은 '{{영업장명}}' 이용 중, 이용료 납부일로부터 ({{미납일수}})일 이상 미납하고 사전 연락 없이 통신이 두절될 경우, 본 시설의 이용 계약을 포기한 것으로 간주함에 명시적으로 동의합니다.\n" +
    // 종전 뒷문장("민사상 손해배상 청구나 형사상 고소 … 어떠한 이의도 제기하지 않을 것을 서약")은
    // 두 겹으로 무효였다(법률 패널 2026-09-09). 고소권 사전 포기는 그 자체로 무효이고, 절도·
    // 주거침입·손괴는 비친고죄라 고소가 없어도 수사된다. 민사 청구 포기는 약관규제법 14조 1호다.
    // 관리자를 실제로 지키는 것은 앞의 동의 자체(피해자 승낙)이므로, 그 승낙을 확인하는 문장으로
    // 바꾼다. 다음 줄은 설명의무 이행 증거라 살린다.
    "또한, 타 입실자의 피해 예방 및 원활한 시설 운영을 위하여, 위 조건에 해당할 시 관리자가 임의로 호실을 개방하는 것에 동의합니다. 아울러 호실 내부에 남겨진 본인의 모든 잔여 물품을 관리자가 임의로 반출, 보관(보관료 1일 3만 원 청구), 처분 및 폐기하는 것에 동의하며, 위 동의 범위 안에서 이루어진 개방·반출·보관·처분은 본인의 승낙에 따른 것임을 확인합니다.\n" +
    "본인은 위 특약의 의미와 법적 효력을 관리자로부터 충분히 설명을 듣고 이해하였으며, 자유로운 의사에 따라 이에 동의합니다.",
}
...
/**
 * 청소비 문장의 **치환 전 문안** 셋 — 참고용 번역본의 사전 열쇠가 이것이다(2026-09-11).
 *
 * 왜 상수로 세우나. 번역 사전의 열쇠는 **치환 전 문안이라야 한다**(`{{일정}}` 과 같은 규칙).
 * 청소비는 계약마다 다르고 계약서에서 표시값으로 덮을 수도 있어, 완성된 문장("청소비 20,000원은…")
 * 을 열쇠로 삼으면 그 사전은 금액이 다른 계약에서 한 번도 안 맞는다. 금액 자리를 `{{청소비}}` 로
 * 남긴 이 문안이 모든 계약에서 같은 하나의 열쇠다.
 *
 * **아래 cleaningFeeVars 가 이 상수를 채워 종이 문장을 만든다.** 두 벌로 두면 문안을 고칠 때
 * 종이만 바뀌고 사전 열쇠는 옛 문장으로 남아, 번역이 통째로 고아가 되고도 아무 소리가 안 난다.
 * `공제` 는 앞 문장에 이어 붙는 꼬리라 값에 앞 한 칸이 붙는다 — 그 칸은 여기 열쇠에 넣지 않는다
 * (환불 규정과 같은 규칙: 열쇠는 문장, 한 칸은 넣는 쪽이 붙인다).
 */
export const CLEANING_FEE_SOURCE = {
  // 성격을 '청소 용역의 대가'로 못박고 두 징수 방식을 함께 적는다(회계 패널 2026-08-02).
  // 애매하면 수익 인식 시점과 부가세 과세 여부 판정이 갈린다.
  clause: '[청소비] 청소비 {{청소비}}은 퇴실 후 실내 청소 용역의 대가입니다. 보증금이 있는 경우 퇴실 정산 시 보증금에서 공제하고, 보증금이 없는 경우 입실 시 이용료와 함께 받습니다.',
  none: '[청소비] 이 계약은 청소비가 없습니다. 퇴실 시 청소비 명목으로 공제하지 않습니다.',
  deduct: '(보증금 내 청소비 {{청소비}} 별도 공제)',
} as const
...
export function cleaningFeeVars(cleaningFee: number | null | undefined): {
  ...
  if (cln <= 0) {
    return {
      청소비: '없음',
      청소비조항: CLEANING_FEE_SOURCE.none,
      청소비공제: '',
    }
  }
  const won = `${cln.toLocaleString()}원`
  // 문장은 위 정본 하나에서 나온다 — 금액만 여기서 채운다. 결과 문자열은 종전과 한 글자도 다르지 않다.
  const fill = (s: string): string => renderContractText(s, { 청소비: won })
  return {
    청소비: won,
    청소비조항: fill(CLEANING_FEE_SOURCE.clause),
    // 앞 문장에 이어 붙는 꼬리라 앞 한 칸이 붙는다. 빼면 "…따릅니다.(보증금 내…" 가 된다.
    청소비공제: ` ${fill(CLEANING_FEE_SOURCE.deduct)}`,
  }
}
...
export type SignedContractSnapshot = {
  ...
  /**
   * 서명 당시 화면에 뜬 참고용 번역본(해석 완료본 한 언어분). 서명은 한국어 정본에만 받지만
   * 입주자는 이 번역을 읽고 이해했다고 말할 것이라, 그 문안이 증거로 얼어야 한다.
   * 이 칸이 생기기 전 박제와 번역본 없는 링크에는 없다(undefined·null) — 그때는 번역본 없음이다.
   */
  translation?: unknown
  /**
