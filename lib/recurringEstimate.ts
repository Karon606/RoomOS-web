...
export type RecurringEstimateSource = {
  ...
  /** 운영자가 고정지출 폼에 손으로 적어 둔 전년 동월 실적. 적지 않았으면 null. */
...
export type RecurringEstimateBasis = 'pending' | 'contract' | 'priorYear' | 'priorYearManual' | 'recentAvg' | 'base'

/**
 * 우선순위: 예약금액 > 비변동 항목의 계약액 > 작년 같은 달 실적 > 수기 전년 동월 > 최근 3개월 평균 > 기본액.
 *
 ...
 *
 * 수기 전년 동월(priorYearAmount)이 실기록 **아래**, 3개월 평균 **위**인 이유(2026-08-19 운영자 지시).
 * 운영자는 이 칸을 "작년 같은 달을 아는 수단"으로 쓴다. 그러니 진짜 작년 기록이 쌓이면 그쪽이 이기고,
 * 아직 없을 때는 계절이 다른 최근 3개월 평균보다 운영자가 아는 작년 값이 낫다. 이 정수에는 연월이
 * 없어 열두 달 내내 같은 값이 쓰이므로 실기록 자리를 빼앗지는 않는다.
 */
export function recurringEstimate(r: RecurringEstimateSource): { amount: number; basis: RecurringEstimateBasis } {
  ...
  if (r.priorYearAmount != null)     return { amount: r.priorYearAmount, basis: 'priorYearManual' }
 ...
 *
 * 실기록과 수기 값을 가르는 표식은 '달이 적혔는가'다. 실기록은 조회월에서 뽑은 '작년 8월'처럼
 * 달을 적고, 수기 값은 연월이 없는 정수라 달을 적으면 거짓이 되므로 폼의 칸 이름 그대로 '전년동월'만
 * 적는다. 운영자는 자기가 그 칸에 적은 값임을 이름으로 알아본다.
 * 라벨 자리는 재무 탭 지출표 '금액' 칸(기본 100px에서 좌우 여백 32px를 뺀 68px)이 가장 좁다.
 */
export function recurringAmountLabel(r: RecurringEstimateSource): string | null {
  switch (recurringEstimate(r).basis) {
    ...
    case 'priorYearManual': return '전년동월'
