// 홈 대시보드가 그리는 DashboardData 한 벌을 만드는 서버 전용 데이터 패칭 모듈
...
export async function getDashboardData(propertyId: string, targetMonth: string) {
  ...
  // 영업장 행은 이 한 번만 읽는다 — 같은 렌더에서 네 곳이 각자 findUnique 를 부르던 자리를
  // 한 칸으로 모았다(연락 리드타임·끈 알림 두 곳). 칸을 더 읽어도 행 하나라 비용은 그대로다.
  const property = await prisma.property.findUnique({
    where: { id: propertyId },
    select: { acquisitionDate: true, prevOwnerCutoffDate: true, cleaningFeeInDeposit: true, contactLeadDays: true, alertMutes: true },
  })
  ...
  // ── 종전에 꼬리에서 한 줄씩 기다리던 조회들 — 앞의 무리와 같은 물결에 태운다.
  // 넷 다 입력이 propertyId·targetMonth·오늘뿐이라 위 조회들의 결과에 기대지 않는다.
  // await 는 각 사용 지점 그대로다(값·계산식·에러 처리 불변, 시작 시점만 앞당김).
  const pPendingDepositLeases = prisma.leaseTerm.findMany({
  ...
  // 위 목록의 id 를 기다리지 않도록 같은 모집단을 관계 where 로 다시 적는다 — 두 조회가 동시에 간다.
  // groupBy 는 그대로 최상위 연산이라 소프트삭제 자동 필터(lib/prisma)가 종전처럼 붙는다.
  const pPendingDepositPaid = prisma.paymentRecord.groupBy({
    by: ['leaseTermId'],
    where: { leaseTerm: { propertyId, status: 'CHECKED_OUT', depositRefunds: { none: {} } }, isDeposit: true },
    _sum: { actualAmount: true },
  })
  ...
  // '연락할 때' 알림(D-14) — 창의 두 입력(오늘 UTC 자정·리드타임)은 여기서 이미 정해진다.
  // KST 오늘의 UTC 자정 — moveInDate 는 @db.Date(UTC 자정)라 같은 기준끼리 비교해야 한다.
  // 로컬 자정이면 서버(UTC)에서 KST 00~09 시에 아직 어제라 D-14 알림이 하루 늦게 뜬다.
  const contactToday = new Date(`${kstYmdStr()}T00:00:00.000Z`)
  const leadDays = property?.contactLeadDays ?? 14
  const pContactLeases = prisma.leaseTerm.findMany({
    where: {
      propertyId, status: { in: ['WAITING_TOUR', 'TOUR_DONE', 'RESERVED'] }, reservationConfirmedAt: null,
      moveInDate: { gte: contactToday },
      OR: [
        { contactAlertDate: { lte: contactToday } },
        { contactAlertDate: null, moveInDate: { lt: new Date(contactToday.getTime() + leadDays * 86400000) } },
      ],
    },
  ...
  for (const p of [pPaidRevenue, pTrendPaidRevenue, pCheckedOutRecognized, pRecurringWithStatus, pExpenseCategories, pDepositRecordedAgg, pDepositCleaningLeases, pReservedDepositReceivedAgg, pReservedExpected, pMoveCalendar, pLastExpAggs, pOverduConfirmed, pPendingDepositLeases, pPendingDepositPaid, pNonRecurringPast, pScheduleMoves, pContactLeases] as Promise<unknown>[]) { void p.catch(() => {}) }
  ...
  const pendingDepositReturns = await (async () => {
    const leases = await pPendingDepositLeases
    if (leases.length === 0) return []
    const paid = await pPendingDepositPaid
  ...
  const nonRecurringPast = await pNonRecurringPast
  ...
  const scheduleMoves = await pScheduleMoves
  {
    const todayYmd = kstYmdStr()
    // 방 번호 표 — 위 방 현황 조회(roomsWithTenants)가 같은 영업장 전 방을 이미 들고 있다.
    // 여기서 또 한 번 읽을 이유가 없다(집합이 같고, 쓰는 것은 id → roomNo 조회뿐이다).
    const roomNos = new Map(roomsWithTenants.map(r => [r.id, r.roomNo]))
  ...
  {
    // 창(contactToday·leadDays)과 조회는 위 조기 시작 무리로 올라갔다 — 리드타임은 이미 읽어 둔
    // 영업장 행에서 나오므로 여기서 영업장을 다시 읽을 이유가 없었다.
    const contactLeases = await pContactLeases
  ...
  try {
    ...
    // 끈 키(alertMutes)는 맨 위에서 읽은 영업장 행에 이미 들어 있다 — 여기서 또 읽지 않는다.
    const crMuteProp = property
...
계좌를 받으면 입주자를 열어 보증금 항목에서 반환 정산을 기록하세요. 기록하면 이 알림이 사라집니다.`,
  ...
  const alertMuteRaw = property?.alertMutes
