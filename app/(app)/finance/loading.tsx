// 지출 관리 전환 즉시 표시되는 로딩 스켈레톤 — 동적 라우트 전환 렉 해결(감사 D6).
// 실제 화면 골격(제목·월셀렉터 → 탭 → 목록)으로 자리를 잡아 레이아웃 점프 방지. 패딩은 AppShell 것만.
  ...
  return (
    <div className="delayed-fallback space-y-4">
      ...
      <div className="flex gap-2">
        <div className="h-10 w-24 rounded-[10px] bg-[var(--cream)] border border-[var(--warm-border)] animate-pulse" />
        <div className="h-10 w-24 rounded-[10px] bg-[var(--cream)] border border-[var(--warm-border)] animate-pulse" />
        <div className="h-10 w-24 rounded-[10px] bg-[var(--cream)] border border-[var(--warm-border)] animate-pulse" />
      </div>
