...
export async function addExpense(formData: FormData) {
  ...
  if (!date || !amount || !category) throw new Error('날짜, 금액, 카테고리는 필수입니다.')
...
export async function addExtraIncome(formData: FormData) {
  await requireEdit()
  const propertyId = await getPropertyId()
  const date      = formData.get('date') as string
  const amount    = parseAmount(formData.get('amount'))
...
export async function updateExtraIncome(formData: FormData) {
  await requireEdit()
  const id        = formData.get('id') as string
  const date      = formData.get('date') as string
  const amount    = parseAmount(formData.get('amount'))
...
export async function saveFinancialAccount(formData: FormData) {
  ...
  if (!brand) throw new Error('금융사명은 필수입니다.')
