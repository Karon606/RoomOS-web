...
// ── 보관 위치 트리 화면 공용 (2026-09-14 5·6단계) ────────────────────────────────────
//
// 서버(getStorageLocations)가 이미 DFS 순으로 주고 각 행이 depth·parentId 를 들고 있다.
...
// 높이·좌우 패딩은 옮기기 모달(TransferStockModal)의 수량칸과 같은 한 벌이다 — 종전 `px-2.5 py-1.5`
// 는 34px 이라, 같은 폼 안에서 점검일 44 · 메모 42 · 수량 34 세 높이가 동시에 섰다(디자이너 지적
// 2026-09-16). §12 는 한 폼 안 입력 높이 혼용을 금지하고 모바일 최소 44 를 요구한다.
const qtyInputCls = 'w-full min-w-0 bg-[var(--canvas)] border border-[var(--warm-border)] rounded-sm px-3 py-2.5 text-sm text-right text-[var(--warm-dark)] outline-none focus:border-[var(--coral)] min-h-[var(--input-h-touch)]'

// 옛 드래프트 호환 — 허브(창고) 칸이 '채운 후'(after)에 묶여 있던 시절의 문서다(2026-09-11 이전).
// 그대로 복원하면 **화면에 안 보이는 값**이 된다(허브 칸은 '잔량'=before 를 그린다). 그 값은
// enteredAny(과잉 입고 신호)와 §12 칩의 스냅샷 비교에만 들어가, 아무것도 안 적었는데 경고가 뜨고
// 배지가 '임시저장 후 수정됨' 으로 앉는다. **드래프트를 읽는 자리마다** 같은 한 줄을 통과시킨다
// (아이템별 문서 · 위치별 문서 둘 다 — 한쪽만 하면 다른 경로로 그 값이 그대로 들어온다).
function hubDraftToBefore(isHubCell: boolean, before: string, after: string): { before: string; after: string } {
  return isHubCell && before === '' && after !== '' ? { before: after, after: '' } : { before, after }
}
...
function locSubtree(all: StorageLocationNode[], id: string): StorageLocationNode[] {
  const start = all.findIndex(n => n.id === id)
  if (start < 0) return []
  const root = all[start]
  const out: StorageLocationNode[] = [root]
  for (let i = start + 1; i < all.length && all[i].depth > root.depth; i++) out.push(all[i])
  return out
}

/** 서브트리의 상대 깊이(자기=0). 옮길 때 자손까지 상한 안인가를 화면이 서버와 같은 규칙으로 센다. */
function locSubtreeSpan(all: StorageLocationNode[], id: string): number {
  const sub = locSubtree(all, id)
  if (sub.length === 0) return 0
  return Math.max(...sub.map(n => n.depth)) - sub[0].depth
}

/**
 * 드롭 판정 밑감 — 들린 블록의 id 집합과 상대 깊이. 드래그 시작(모달 열림)에 **한 번** 센다.
 * `blocked` 는 `wouldCycle` 이 보는 그 집합(자기 + 자손)이라 판정이 서버와 같은 규칙이다.
 */
type LocDropCtx = {
  all: StorageLocationNode[]
  node: StorageLocationNode
  blocked: Set<string>
  span: number
}
const locDropCtx = (all: StorageLocationNode[], node: StorageLocationNode): LocDropCtx =>
  ({ all, node, blocked: new Set(subtreeIds(all, node.id)), span: locSubtreeSpan(all, node.id) })

/**
 * 한 자리(새 부모 + 적용 후 이름)가 막혔는가. 막혔으면 이유 한 줄과 걸린 이름, 아니면 null.
 *
 * **드래그와 옮기기 모달이 같은 이 함수를 부른다.** 사본을 만들면 화면이 막은 자리와 서버가 실제로
 * 거부하는 자리가 언젠가 갈린다 — 규칙도 순서도 서버(placeStorageLocation)와 같게 둔다.
 * `samePlace` 만 입구마다 뜻이 다르다. 모달은 '같은 부모면 참', 드래그는 '같은 부모 **같은 자리**일
 * 때만 참' 이다(형제 안 순서 바꾸기는 드래그에서 유효한 드롭이다).
 */
function reasonOf(
  ctx: LocDropCtx, parentId: string | null, nextName: string, samePlace: boolean,
): { text: string; clash: string | null } | null {
  if (samePlace) return { text: '지금 있는 자리입니다', clash: null }
...
/**
 * 이름 제안 기본값 — **방향별**이다(운영자 결정 2026-09-16). 얕아지되 부모가 남으면 켜고,
 * 최상위로 빼면 끈다 — 루트로 뺄 때 켜면 옛 경로 전체가 이름이 되어 대개 과하다.
 */
const locPreserveDefault = (parentId: string | null) => parentId !== null

/** 체크박스 라벨 — 규칙은 하나(pathName 보존)고 라벨만 방향을 말한다. 줄면 떼기, 늘면 붙이기. */
const locPreserveLabel = (from: string, to: string) => (to.length > from.length ? '앞부분 붙이기' : '앞부분 떼기')

/** 옮긴 자리를 말하는 한 조각. 이름은 따옴표 안에 두고 조사는 뒤따르는 명사가 받는다. */
const locDestLabel = (parentPath: string | null, index: number, siblingCount: number) =>
  `${parentPath === null ? '최상위' : `'${parentPath}' 아래`} ${
...
/** 드롭 후보 한 자리. **바뀔 때만** state 로 올린다(포인터 이동마다 올리면 목록이 다시 그려진다). */
type LocDrop = {
  parentId: string | null
  index: number                 // 들린 블록을 뺀 형제 배열에서의 자리
  depth: number                 // 놓이면 갖게 될 깊이(표시선 들여쓰기의 근거)
  anchorId: string | null       // 표시선 기준 행(null = 목록 끝·제자리)
...
/** 히트 판정용 행 한 줄 — pointerdown 에서 한 번 캐시(목록 기준 offset, 드래그 중 불변). */
type LocHitRow = { id: string; depth: number; top: number; bottom: number; subtreeBottom: number }
...
/** 화면이 그릴 몫 — 자리표 높이와 고스트에 담을 머리 행. 드래그 시작에 한 번만 정해진다. */
type LocDragView = {
  id: string
  ids: Set<string>
  height: number
  ...
  more: number
  rootDepth: number
  left: number; width: number; y0: number
}
...
function locHitOf(s: LocDragSession, y: number, all: StorageLocationNode[]): LocDrop | null {
  const { ctx, rows } = s
  const node = ctx.node
  const half = LOC_GAP_PX / 2
  const sibsOf = (pid: string | null) => all.filter(n => n.parentId === pid && n.id !== node.id).map(n => n.id)
  const ownIdx = all.filter(n => n.parentId === node.parentId).findIndex(n => n.id === node.id)

  const make = (parentId: string | null, index: number, depth: number, anchorId: string | null,
                mode: LocDrop['mode'], lineY: number): LocDrop => {
    const parentPath = parentId === null ? null : (all.find(n => n.id === parentId)?.pathName ?? '')
    const suggested = preserveName(node.pathName, parentPath)
    // 이름 제안은 방향별 기본값 그대로다 — 드래그 중에는 체크박스를 누를 손이 없다(고스트 아래
    // 한 줄은 그 기본값을 미리 보여 주는 자리고, 바꾸려면 관리 > 옮기기를 쓴다).
    const nextName = locPreserveDefault(parentId) && suggested != null ? suggested : node.name
    const samePlace = mode === 'same' || (parentId === node.parentId && index === ownIdx)
  ...
  // 들린 블록이 비운 자리 — 되돌려 놓는 드롭이다(막힘으로 표시하되 토스트는 없다).
  if (y >= s.blockTop - half && y < s.blockBottom + half) {
    return make(node.parentId, ownIdx, node.depth, null, 'same', s.blockTop - half)
  }
  if (rows.length === 0) return make(null, 0, 1, null, 'end', s.blockBottom + half)
  const last = rows[rows.length - 1]
  if (y >= last.bottom + half) return make(null, sibsOf(null).length, 1, null, 'end', last.bottom + half)
  const first = rows[0]
  const head = all.find(n => n.id === first.id)
  if (!head) return null
  return make(head.parentId, 0, first.depth, first.id, 'before', first.top - half)
}
...
function DetailModalSkeleton({ mode }: { mode: 'view' | 'check' }) {
  ...
  // `px-5 sm:px-6 pt-3` 이라, 골격에 한 벌만 쓰면 640px 이상에서 첫 페인트가 4px 튄다.
  return mode === 'check' ? (
    <div className="px-5 py-4 space-y-3 delayed-fallback" aria-busy="true" aria-label="불러오는 중">
      {/* 안내문 두 줄 — 실제 폼의 첫 블록이다. 빼면 골격이 통째로 위로 당겨 붙는다. */}
      ...
      {/* 위치 행 두 개 — 실제 비허브 행의 다섯 단(제목 16 · 참고줄 23 · 라벨 15 · 입력 44 ·
          옮김 없음 25)을 그대로 세운다. 종전 76px 짜리 모형은 실제(약 140px)의 절반이라
          로디드 순간 세로가 통째로 늘어났다. 행 개수 2 는 고정이다 — 실제 위치 수를 모르는 자리다. */}
      {[0, 1].map(i => (
        <div key={i} className="space-y-1 pb-2">
          <div className={`h-4 w-28 ${bar}`} />
          <div className={`h-[23px] ${bar}`} />
    ...
    <div className="px-5 sm:px-6 pt-3 space-y-3 delayed-fallback" aria-busy="true" aria-label="불러오는 중">
      ...
      <div className="space-y-1">
        ...
        {unreceived ? (
          ...
          <div className="flex gap-2 items-center">
            <DatePicker value={receivedDate} onChange={setReceivedDate} className="flex-1 bg-[var(--canvas)] border border-[var(--warm-border)] rounded-sm px-3 py-2 text-sm text-[var(--warm-dark)]" />
...
function CheckForm({ item, lastCheckBreakdown, lastCheckCreatedAt, hiddenLocationIds, currentStock, hasPriorCheck, pendingCount, onCancel, onDone, onDraftChange, onGoDisposal }: {
  ...
  lastCheckCreatedAt?: Date | string | null   // 참고줄의 '저장된 잔량'이 언제 저장된 값인지 — §11 메타 칩
  ...
  useEffect(() => {
    ...
    getItemDrafts(item.id).then(drafts => {
      ...
      if (hubLoc) {
        const moved = hubDraftToBefore(true, beforeOv[hubLoc.id] ?? '', afterOv[hubLoc.id] ?? '')
        if (moved.before !== (beforeOv[hubLoc.id] ?? '')) { beforeOv[hubLoc.id] = moved.before; delete afterOv[hubLoc.id] }
      }
      ...
      for (const dr of drafts) {
        ...
        // 구 문서 호환(위치별 문서) — 여기에도 같은 한 줄을 둔다. 한쪽만 하면 이 경로로 그 값이
        // 그대로 들어와, 화면에 안 보이는 채 과잉 입고 신호와 '수정됨' 배지를 켠다.
        const { before: drBefore, after: drAfter } = hubDraftToBefore(
          hubLoc != null && dr.locationId === hubLoc.id,
          dd.before != null ? String(dd.before) : '',
          dd.after != null ? String(dd.after) : '',
        )
        if (drBefore !== '' && (newer || beforeOv[dr.locationId] == null)) { beforeOv[dr.locationId] = drBefore; savedAtOf[dr.locationId] = Math.max(savedAtOf[dr.locationId] ?? 0, sv) }
        if (drAfter  !== '' && (newer || afterOv[dr.locationId] == null))  { afterOv[dr.locationId]  = drAfter;  savedAtOf[dr.locationId] = Math.max(savedAtOf[dr.locationId] ?? 0, sv) }
      ...
      for (const [id, sv] of Object.entries(savedAtOf)) {
        const b = beforeOv[id] ?? '', a = afterOv[id] ?? ''
        if (b === '' && a === '') continue
        restoredRows[id] = { savedAt: sv, before: b, after: a }
      }
  ...
  const handleClearDraft = () => {
    ...
    // 반환값을 읽는다 — 안 지워졌는데 '비움'이라고 말하면 다음 진입에서 그 값이 되살아난다.
    deleteItemDrafts(item.id).then(res => {
      onDraftChange?.()
      if (!res.ok) { pushToast('error', res.error); return }
  ...
  // 저장 뒤 드래프트 정리 — 반환값을 읽는다. 저장은 됐는데 임시저장이 남으면 다음 진입에서
  // 그 값이 되살아나 실저장분과 섞인다(위치 패널과 같은 규칙, 신고 2026-09-14).
  const clearDraftsAfterSave = async () => {
    const del = await deleteItemDrafts(item.id)
    onDraftChange?.()
    if (!del.ok) {
      pushToast('error', '저장은 됐지만 임시저장이 남았습니다', { detail: "임시저장 칩의 '비우기'로 지워 주세요." })
    }
  }
  ...
  // 저장 성공 토스트의 부가 옵션 — 세 갈래가 **한 곳**에서 만든다(문장·조건이 갈리면 어느 갈래만
  // 되돌릴 수 없게 된다).
  //  · sameDayNotice 는 별도 토스트가 아니라 이 토스트의 detail 이다(§15 겹쳐 쌓지 않는다).
  //  · deduped 면 **적용취소를 붙이지 않는다**. 그때 돌아온 id 는 이 제출이 만든 점검이 아니라
  //    20초 안에 있었던 다른 저장이다 — 지우면 남의 기록을 지운다(검수 지적 2026-09-16).
  //  · 적용취소가 끝나면 결과 토스트를 낸다(§16 Do — 적용취소 후에도 결과 토스트).
  const checkSaveToastOpts = (res: { id: string; sameDayNotice?: boolean; deduped?: boolean }) => ({
    ...
    ...(res.deduped
      ...
      : {
        action: { label: '적용취소', run: () => { void deleteStockCheck(res.id).then(r => {
          if (r.ok) { onDraftChange?.(); onDone(); pushToast('info', '재고 점검 저장을 적용취소했습니다') }
          else pushToast('error', r.error)
        }) } },
      }),
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!hasLocations) {
      ...
      startTransition(async () => {
        try {
          ...
          pushToast('success', '재고 점검 저장됨', checkSaveToastOpts(res))
          onDone()
    ...
    if (restockMode) {
      ...
      //
      // **허브를 직접 셌으면 부족 게이트를 끈다**(`allowHubClamp: hubMeasured`). 위치 패널이 같은
      // 저장 안에 허브 실측 단위가 있으면 비허브 부족 판정을 건너뛰는 것과 **같은 술어**다
      // (measuredHubs·clampRef). 지금 세는 허브 값이 곧 정답이고 허브 패치가 마지막에 그 값을
      // 덮으므로, 장부상 부족을 이유로 팝업을 띄우면 화면이 거짓말을 한다 — 검수 재현 A 에서
      // `others` 가 비어 "옮겨올 재고가 있는 위치가 없습니다" 가 떴다(2026-09-16).
      const saveArgs = {
        trackedItemId: item.id, date, remainingQty: 0, memo: memo || undefined,
        locationPatches, isReconcile: reconcileMode, allowHubClamp: hubMeasured,
      }
        ...
        try {
          ...
          if (!res.ok) {
            if ('code' in res && res.code === 'HUB_SHORT') {
              // 막힌 자리를 **이름으로** 말한다 — 팝업 머리의 품목 자리에 그 칸 이름이 선다.
              // 인덱스가 아니라 위치 id 를 읽는다(서버가 허브를 맨 뒤로 민 뒤의 자리라 우리 배열과 어긋난다).
              const stuckName = res.stuckLocationId
                ? (chkLocations.find(l => l.id === res.stuckLocationId)?.pathName ?? '')
                : ''
    ...
    <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3 flex-1 overflow-y-auto">
          ...
          {chkLocations.map(loc => {
            ...
            return (
              <div key={loc.id} className="space-y-1 border-b border-[var(--warm-border)]/40 pb-2 last:border-0">
                {/* 창고 표시는 §22 정본 Badge 다 — 위치 패널 그룹 헤더의 `기본 창고` 와 같은 뱃지·같은 톤.
                    평문 `(창고)` 로 쓰면 같은 사실을 두 화면이 다른 모양으로 말한다(디자이너 검수 2026-09-16). */}
                <div className="flex items-baseline gap-1.5">
                  <p className="min-w-0 truncate text-xs font-medium text-[var(--warm-dark)]">{loc.pathName}</p>
                  {rowIsHub && <Badge tone="pale-amber" className="shrink-0">기본 창고</Badge>}
                </div>
      ...
      <div className="space-y-1.5">
        ...
        <input type="text" value={memo} onChange={e => setMemo(e.target.value)}
          className="w-full bg-[var(--canvas)] border border-[var(--warm-border)] rounded-sm px-3 py-2.5 text-sm text-[var(--warm-dark)] outline-none min-h-[var(--input-h-touch)]" />
      </div>
      ...
      {(draftPending || draftSavedAt) && (() => {
        ...
        return (
          <div className="flex items-center justify-between gap-2 text-[0.65625rem] rounded-lg px-2.5 py-1.5" style={{ background: 'var(--cream-2)' }}>
            ...
            {/* 히트영역은 §25 유사요소 확장으로 44px — 글자만으로는 27x13px 이다(위치 패널과 같은 문법). */}
            {!draftPending && draftSavedAt && (
              <button type="button" onClick={handleClearDraft}
                className="relative text-[var(--warm-muted)] hover:text-[var(--warm-dark)] underline shrink-0 before:absolute before:content-[''] before:-inset-x-3 before:-inset-y-[15px]">비우기</button>
            )}
          </div>
        )
      })()}
      ...
      {hubShort && (
        <HubShortDialog key={hubShort.trackedItemId} pending={hubShort}
          onResolved={() => { setHubShort(null); void clearDraftsAfterSave().then(() => onDone()) }}
...
// 옮기기 완료 결과 — 호출부가 두 쌍(품목×출발지, 품목×도착지)의 미저장 입력을 무효로 만드는 데 쓴다.
// 옮긴 순간 그 두 칸에 대해 '옮기기 전에 센 값'은 더 이상 사실이 아니기 때문이다(2026-09-15 패널 확정).
type TransferDone = {
  checkId: string
  trackedItemId: string
  fromId: string
...
function TransferStockModal({ rows, onClose, onDone, initialItemId, initialFromId, lockItem, silent, z }: {
  ...
  useEffect(() => { if (initialItemId) void pickItem(initialItemId, initialFromId) }, [])   // eslint-disable-line react-hooks/exhaustive-deps
  const [locStock, setLocStock] = useState<ItemLocationStock[] | null>(null)
  const [fromId, setFromId] = useState('')
  const [toId, setToId] = useState('')
  const [qtyStr, setQtyStr] = useState('')
  const [srcLeftStr, setSrcLeftStr] = useState('')   // 옮긴 뒤 출발지 남은 양(선택 실측)
  ...
  // 남은 양은 **안 적는 것이 기본**이다(빈칸 = 장부에서 뺀 값). 0 은 '남은 게 없다'는 실측이므로
  // 빈 문자열과 구별한다 — placeholder 로 0 을 미리 보이면 그 둘이 화면에서 섞인다.
  const srcLeft = (!swapMode && srcLeftStr !== '') ? Number(srcLeftStr) : null
  const srcLeftValid = srcLeft == null || (Number.isFinite(srcLeft) && srcLeft >= 0)
  const canSubmit = !!item && !!fromLoc && !!toLoc && fromId !== toId && srcLeftValid
    && (swapMode ? ((fromLoc?.qty ?? 0) > 0 || (toLoc?.qty ?? 0) > 0) : (moveQty > 0 && moveQty <= (fromLoc?.qty ?? 0)))

  const pickItem = async (id: string, presetFromId?: string) => {
    setItemId(id); setLocStock(null); setFromId(''); setToId(''); setQtyStr(''); setSrcLeftStr(''); setSwapMode(false)
    if (!id) return
    const res = await getItemLocationStock(id)
    if (res.ok) {
      setLocStock(res.locations)
      // 행 진입의 출발지 — 그 품목의 위치 목록에 있을 때만 세운다(없으면 종전처럼 고르게 둔다).
      if (presetFromId && res.locations.some(l => l.id === presetFromId)) setFromId(presetFromId)
    }
    else pushToast('error', res.error)
  }
  ...
  const submit = async () => {
    ...
    try {
      const res = await transferLocationStock({
        ...
        ...(swapMode ? { swap: true } : { qty: moveQty, ...(srcLeft != null ? { sourceRemainingQty: srcLeft } : {}) }),
      })
      if (!res.ok) { pushToast('error', res.error); return }
          ...
          <>
              ...
              <div className="flex flex-wrap gap-1.5">
                {locStock.filter(l => l.qty > 0 || swapMode).map(l => (
                  <button key={l.id} type="button" className={chip(fromId === l.id)}
                    // 남은 양은 출발지에 매인 값이다 — 출발지를 바꾸면 앞 위치의 실측이 남으면 안 된다.
                    onClick={() => { setFromId(l.id); setSrcLeftStr(''); if (toId === l.id) setToId('') }}>
            ...
            <label className="flex items-center gap-2 text-xs text-[var(--warm-dark)] cursor-pointer">
              <input type="checkbox" checked={swapMode}
                onChange={e => { setSwapMode(e.target.checked); setQtyStr(''); setSrcLeftStr('') }}
            ...
            {fromLoc && toLoc && canSubmit && (
              <div className="rounded-xl border border-[var(--warm-border)] bg-[var(--canvas)] px-3.5 py-2.5 text-xs text-[var(--warm-dark)] space-y-0.5">
                ...
                {swapMode ? (
                  ...
                  const ledgerLeft = fromLoc.qty - moveQty
                  const nextLeft = srcLeft ?? ledgerLeft
                  const diff = Math.round((nextLeft - ledgerLeft) * 100) / 100
                  return (
                    <>
                      <p>{fromLoc.pathName}: {fmtQty(fromLoc.qty, unit)} → <strong>{fmtQty(nextLeft, unit)}</strong></p>
                      {diff !== 0 && (
...
// ── 위치별 점검 패널 — 서브트리 한 덩어리 점검 (2026-09-14 6단계)
//
// 고른 위치의 **자손까지 한 화면**에 펼친다. 9/11 김치 사고가 "냉장고 상단·하단을 따로 놔서
// 한 번에 체크를 못 한" 사고였고, 그 분류를 깜빡한 대가로 전날 내역을 나중에 고쳐야 했다.
//
// 저장은 **체인**이다. (품목, 위치) 쌍을 DFS 순으로 하나씩 병합 저장하고, 같은 품목의 **허브 행은
// 맨 마지막**에 둔다. 서버는 무변경이다 — 같은 날 연속 점검 병합 정본(applyLocationCheck)을
// 그대로 타므로 마커 보존·이월 표식(carried)·허브 게이트 규칙이 한 줄도 안 바뀐다.
//
// 왜 순차인가. 한 품목의 두 번째 위치부터는 **이 저장에서 만든 점검**에 이어 붙여야 한다.
// 병렬로 쏘면 위치마다 새 점검이 서고, 그 각각이 직전 점검을 base 로 잡아 앞 위치의 실측이
// 조용히 사라진다. 비원자성(상단 저장 후 하단 실패)은 체인을 멈추고 **어디까지 저장됐는지**를
// 남기는 것으로 받는다(설계 결정 2026-09-14, 원자 저장은 후속).
type LocSaveUnit = {
  r: InventoryRow
  locId: string
  locName: string        // 표기 경로(pathName) — 실패·물음 목록이 어느 칸인지 말해야 한다
  isHubUnit: boolean     // 이 위치가 그 품목의 허브인가(허브 실측 행)
}
...
function LocationBatchCheckModal({ rows, onClose = () => {}, onDone, inline = false, onDraftChange }: {
  ...
  const [locId, setLocId] = useState('')                    // 고른 서브트리의 루트
  const [transferOpen, setTransferOpen] = useState(false)   // 위치 간 이동·맞바꿈(운영자 요청 2026-07-08)
  // 행에서 연 옮기기 — 품목·출발지가 이미 정해진 진입(2026-09-15). 헤더 '위치 이동'과 다른 자리다.
  const [rowTransfer, setRowTransfer] = useState<{ itemId: string; fromId: string } | null>(null)
  const [date, setDate] = useState(kstYmdStr())
  const [pending, setPending] = useState(false)
  const [draftPending, setDraftPending] = useState(false)
  ...
  // 허브 부족 감지 시 처리 대기 큐. 체인은 **한 건에서 멈추고** 팝업이 그 건을 처리한 뒤 이어 달린다.
  const [hubShortQueue, setHubShortQueue] = useState<HubShortPending[]>([])
  // 저장 실패 행 — 하나라도 있으면 패널을 닫지 않는다(§27.2). 종전엔 setError 만 하고 그대로
  // 닫아서(인라인 모드의 onClose = 화면 전환) 에러가 한 프레임도 안 그려졌고, 실패한 품목의
  // 임시저장만 남아 "실저장을 눌렀는데 임시저장이 남아 있다"가 됐다(운영자 신고 2026-09-14).
  const [saveFailed, setSaveFailed] = useState<{ id: string; label: string; error: string }[]>([])
  // 체인이 어디까지 갔는가 — 비원자 저장의 값이다. 멈춘 자리를 말하지 않으면 운영자는 무엇을
  // 다시 눌러야 하는지 알 수 없다(스크롤 밖 고정 영역이 그린다).
  const [saveProgress, setSaveProgress] = useState<{ done: number; total: number; stoppedAt: string } | null>(null)
  // 저장은 됐는데 임시저장 정리에 실패한 행 — 남은 드래프트가 다음 진입에서 실저장분과 섞인다.
  const [draftCleanupFailed, setDraftCleanupFailed] = useState<string[]>([])
  // (품목,위치)별 드래프트 기준값 — 행 캡션('임시저장 HH:MM' / '임시저장 후 수정됨')의 판정 근거.
  const [rowDrafts, setRowDrafts] = useState<Record<string, { savedAt: number; before: string; after: string; itemId: string; locId: string }>>({})
  // 이중 차감 물음 대상 — 형제 confirmItems 와 같은 자리의 인라인 바가 한 번만 묻는다.
  const [dupUnits, setDupUnits] = useState<LocSaveUnit[]>([])
  // 접힘은 비영속·기본 펼침이다(설계 결정) — 9/11 사고가 '칸을 깜빡한' 사고라 접힌 채 다음 날
  // 열리면 같은 사고가 난다. 접힌 그룹에 입력값이 있으면 헤더에 '입력 N' 캡션이 선다.
  const [collapsed, setCollapsed] = useState<Set<string>>(new Set())
  // 비우기 적용취소는 6초 뒤에 눌릴 수 있다 — 그때의 '지금 위치'를 봐야 다른 위치의 패널에
  // 엉뚱한 칩·캡션을 세우지 않는다. 렌더마다 최신값으로 맞춘다(쓰기만 하는 ref).
  ...
  const scopeItemCount = groups.reduce((s, g) => s + g.items.length, 0)

  // 위치별 "보충 전" + "보충 후" — 키는 (품목, 위치) 쌍이다. 한 품목이 여러 칸에 서기 때문이다.
  const [beforeQtys, setBeforeQtys] = useState<Record<string, string>>({})
  const [afterQtys, setAfterQtys]   = useState<Record<string, string>>({})
  ...
  useEffect(() => {
    ...
    setMergeChoice(null)
    setConfirmItems([])
    // 위치를 바꾸면 앞 위치의 실패·드래프트 표시는 전부 초기화한다 — 남으면 다른 위치의 사실을
    // 이 위치의 것처럼 말하게 된다.
    setSaveFailed([])
    setSaveProgress(null)
    setDraftCleanupFailed([])
    setRowDrafts({})
    setDupUnits([])
    ...
    void getLocationDraftsFor(ids).catch(() => [] as { locationId: string; trackedItemId: string; data: { before?: string; after?: string; savedAt: number } }[]).then(drafts => {
      ...
      for (const d of drafts) {
        ...
        const row = rows.find(r => r.id === d.trackedItemId)
        const isHubCell = row?.locations.find(l => l.id === lid)?.isHub ?? false
        // 옛 문서(허브 값이 '채운 후'에 든 것)는 '잔량' 칸으로 옮겨 읽는다 — 정본 한 줄.
        const { before, after } = hubDraftToBefore(
          isHubCell,
          d.data?.before != null ? String(d.data.before) : '',
          d.data?.after != null ? String(d.data.after) : '',
        )
        ...
        if (before !== '') nextBefore[k] = before
        if (after  !== '') nextAfter[k]  = after
        if (savedAt > latest) latest = savedAt
      }
      if (Object.keys(base).length === 0) return
      setRowDrafts(base)
      // 재진입 시에도 칩을 세운다 — 아이템별 폼(CheckForm)은 복원값으로 칩을 세우는데 여기만
      // 안 세워서, 임시저장이 남아 있어도 비울 길이 화면에 없었다(형제 갈림).
      if (latest > 0) setLocDraftSavedAt(latest)
      setBeforeQtys(prev => ({ ...prev, ...nextBefore }))
      setAfterQtys(prev => ({ ...prev, ...nextAfter }))
    })
  }, [locId]) // eslint-disable-line react-hooks/exhaustive-deps

  // 점검 위치가 비허브일 때, 각 품목당 보충량은 후-전, 합계만큼 그 품목의 허브 위치 잔량 자동 차감
  const computeRow = (r: InventoryRow, lid: string) => {
    const k = locPairKey(r.id, lid)
    const beforeStr = beforeQtys[k] ?? ''
    const afterStr  = afterQtys[k] ?? ''
  ...
  const isPairDirty = (r: InventoryRow, lid: string) => {
    const { beforeStr, afterStr } = computeRow(r, lid)
    return beforeStr !== '' || afterStr !== ''
  }

  // 저장 단위 — DFS 순으로 훑고, 같은 품목의 **허브 행을 맨 뒤로** 민다.
  // Array.prototype.sort 는 안정 정렬이라 나머지 순서는 DFS 그대로다.
  const buildUnits = (dirtyOnly = true): LocSaveUnit[] => {
    const units: LocSaveUnit[] = []
    for (const g of groups) {
      for (const r of g.items) {
        if (dirtyOnly && !isPairDirty(r, g.node.id)) continue
        units.push({
          r, locId: g.node.id, locName: g.node.pathName,
          isHubUnit: r.locations.find(l => l.id === g.node.id)?.isHub ?? false,
        })
      }
    }
    return units.sort((a, b) => (a.isHubUnit ? 1 : 0) - (b.isHubUnit ? 1 : 0))
  }

  const dirtyUnitCount = buildUnits().length
  const totalRestock = groups.reduce((s, g) => s + g.items.reduce((s2, r) => s2 + computeRow(r, g.node.id).restocked, 0), 0)

  // 한 품목이 이번 저장에서 **제 창고에서 빼내는 양** — 허브 행 참고줄의 '차감 후' 가 쓴다.
  // 허브 칸 자신의 입력은 옮김이 아니므로 뺀다(창고에서 창고로 옮기는 일은 없다).
  const itemRestockSum = (r: InventoryRow) => groups.reduce((s, g) => {
    if (!g.items.some(x => x.id === r.id)) return s
    if (r.locations.find(l => l.id === g.node.id)?.isHub) return s
    return s + computeRow(r, g.node.id).restocked
  }, 0)

  // 같은 점검에 이미 보충 마커가 있는데 이번에도 보충이 계산되는가 — 이중 차감의 유일한 입구다.
  // 판정 축은 아래 saveUnit 의 머지 조건과 같아야 한다(새 점검으로 저장되면 마커가 안 겹친다).
  const doubleRestockOf = (r: InventoryRow, lid: string, forceMerge?: boolean) => {
    const { restocked } = computeRow(r, lid)
    if (restocked <= 0) return null
    const sameDay = r.lastCheckId && r.lastCheckCreatedAt && isSameKstDay(new Date(r.lastCheckCreatedAt), new Date())
    const within6h = r.lastCheckCreatedAt && (Date.now() - new Date(r.lastCheckCreatedAt).getTime()) < 6 * 3600_000
    if (!(date === kstYmdStr() && (forceMerge || (sameDay && within6h)))) return null
    const prevRestocked = r.currentLocationBreakdown.find(lb => lb.locationId === lid)?.restockedQty ?? 0
    if (prevRestocked <= 0) return null
    return { prevRestocked, restocked }
  }

  // ── 체인 상태 — 팝업으로 끊겼다가 이어 달리려면 렌더 밖에 들고 있어야 한다.
  const chainIdsRef   = useRef<Map<string, string>>(new Map())   // 품목 id → 이 저장에서 쓴 점검 id
  const clampRef      = useRef<Set<string>>(new Set())           // 부족 게이트를 건너뛸 (품목,위치) 쌍
  ...
  const saveUnit = (u: LocSaveUnit, forceMerge: boolean, opts?: { allowHubClamp?: boolean; forceNew?: boolean }) => {
    ...
    const hubLoc = u.r.locations.find(l => l.isHub)
    const locationPatch = {
      checkedLocationId: u.locId,
      afterQty: finalN ?? 0,
    ...
    const sameDay = u.r.lastCheckId && u.r.lastCheckCreatedAt && isSameKstDay(new Date(u.r.lastCheckCreatedAt), new Date())
    const within6h = u.r.lastCheckCreatedAt && (now - new Date(u.r.lastCheckCreatedAt).getTime()) < 6 * 3600_000
    // 체인 — 같은 품목의 두 번째 칸부터는 **이 저장에서 만든/쓴 점검**에 이어 붙인다.
    // 안 이어 붙이면 칸마다 새 점검이 서고 그 각각이 직전 점검을 base 로 잡아 앞 칸의 실측이 사라진다.
    const chained = chainIdsRef.current.get(u.r.id) ?? null
  ...
  const runChain = async (units: LocSaveUnit[], forceMerge: boolean, doneBefore: number, total: number):
    ...
    for (let i = 0; i < units.length; i++) {
      ...
      catch (e) { res = { ok: false as const, error: humanError(e, '저장 중 오류가 발생했습니다.') } }
      if (res.ok) {
        // **반환값을 읽는다** — 지운 줄 알았는데 남아 있으면 다음 진입에서 그 값이 되살아나
        // 실저장분과 섞인다. 그것이 신고 2026-09-14 의 증상 그 자체다.
        const del = await deleteStockCheckDraft(u.r.id, u.locId).catch(() => ({ ok: false as const, error: '' }))
      ...
      if ('code' in res && res.code === 'HUB_SHORT') {
        ...
        return { stopped: 'hubShort', done }
      }
      setSaveFailed([{ id: locPairKey(u.r.id, u.locId), label, error: humanError(res.error, '저장 중 오류가 발생했습니다.') }])
      setSaveProgress({ done, total, stoppedAt: label })
      chainCtxRef.current = { forceMerge, done, total, stoppedAt: label }
      return { stopped: 'failed', done }
    }
    return { stopped: null, done }
  }

  // 체인이 멈추거나 끝난 뒤 화면 상태를 한 번에 맞춘다(정리된 드래프트·칩·정리 실패 목록).
  const settleChain = () => {
    const cleaned = cleanedKeysRef.current
    if (cleaned.size > 0) {
      setRowDrafts(prev => {
        const next = { ...prev }
        cleaned.forEach(k => { delete next[k] })
        return next
      })
      if (Object.keys(rowDraftsRef.current).every(k => cleaned.has(k))) {
        setLocDraftSavedAt(null)
        locDraftSnapRef.current = null
      }
    }
    setDraftCleanupFailed([...cleanupRef.current])
    onDraftChange?.()
  }

  // dupDecision — 이중 차감 물음의 답. 'keep' 더 옮긴 것으로 저장 / 'skip' 그 행만 빼고 저장.
  // 상태가 아니라 인자로 흐른다(형제 mergeChoice 바와 같은 문법 — setState 는 이 호출에 안 보인다).
  const doSave = async (forceMerge?: boolean, dupDecision?: 'keep' | 'skip') => {
    if (savingRef.current) return   // 이미 저장 진행 중 — 중복 제출(더블클릭) 차단
    savingRef.current = true
    try {
      let units = buildUnits()
      if (units.length === 0) { setError('저장할 수량이 없습니다.'); return }

      // 이중 차감 물음 — 행마다 모달을 N연타로 띄우지 않고, 형제 '기존에 합치기' 바와 같은
      // 자리에서 한 번만 묻는다. 서버 규칙은 안 건드린다.
      const dups = units.filter(u => doubleRestockOf(u.r, u.locId, forceMerge) != null)
      if (dups.length > 0 && !dupDecision) { setDupUnits(dups); return }
      const skip = dupDecision === 'skip' ? new Set(dups.map(u => locPairKey(u.r.id, u.locId))) : new Set<string>()
      units = units.filter(u => !skip.has(locPairKey(u.r.id, u.locId)))
      setDupUnits([])
      if (units.length === 0) { setError('저장할 행이 없습니다.'); return }

      setPending(true); setError('')
      setSaveFailed([]); setDraftCleanupFailed([]); setSaveProgress(null)
      chainIdsRef.current = new Map()
      cleanupRef.current = []
      ...
      // (HubShortDialog 의 '허브 실측' 선택지를 앞당겨 적용한 것이다). 허브 실측이 없으면
      // 게이트는 오늘과 똑같이 선다.
      const measuredHubs = new Map<string, Set<string>>()
      for (const u of units) {
        if (!u.isHubUnit) continue
        const s = measuredHubs.get(u.r.id) ?? new Set<string>()
        s.add(u.locId)
        measuredHubs.set(u.r.id, s)
      }
      clampRef.current = new Set(units.filter(u => {
        if (u.isHubUnit) return false
        const hubId = u.r.locations.find(l => l.isHub)?.id
  ...
  // 팝업이 멈춘 한 건을 처리(저장)했을 때 — 그 행의 드래프트를 정리하고 **체인을 이어 달린다**.
  const onHubShortResolved = async () => {
    const cur = stoppedUnitRef.current
    stoppedUnitRef.current = null
    setHubShortQueue([])
    let done = chainCtxRef.current.done
    if (cur) {
      const del = await deleteStockCheckDraft(cur.r.id, cur.locId).catch(() => ({ ok: false as const, error: '' }))
      if (del.ok) cleanedKeysRef.current.add(locPairKey(cur.r.id, cur.locId))
      else cleanupRef.current.push(`${cur.r.label} · ${cur.locName}`)
      done++
    }
    const rest = restUnitsRef.current
    restUnitsRef.current = []
    const out = rest.length > 0
  ...
  const handleSave = async () => {
    ...
    // 물음은 **품목 단위**다 — 한 품목이 여러 칸에 서도 합칠 점검은 하나다.
    const now = Date.now()
    const seen = new Set<string>()
    const needsConfirm = date === kstYmdStr() ? units.filter(u => {
      if (seen.has(u.r.id)) return false
      const hit = !!(u.r.lastCheckId && u.r.lastCheckCreatedAt &&
        isSameKstDay(new Date(u.r.lastCheckCreatedAt), new Date()) &&
        (now - new Date(u.r.lastCheckCreatedAt).getTime()) >= 6 * 3600_000)
      if (hit) seen.add(u.r.id)
      return hit
    }).map(u => u.r) : []
  ...
  // 위치별 임시저장 — 입력값이 있는 (품목, 위치) 쌍마다 드래프트 저장.
  // 반환값을 읽는 문법은 아이템별 폼(CheckForm.handleSaveDraft)과 같다.
  const handleSaveDraft = async () => {
    if (!locId) return
    const units = buildUnits()
    if (units.length === 0) { setError('임시저장할 수량이 없습니다.'); return }
    setDraftPending(true); setError('')
    const savedAt = Date.now()
    try {
      const settled = await Promise.allSettled(units.map(u => {
        const { beforeStr, afterStr } = computeRow(u.r, u.locId)
        return saveStockCheckDraft({
          trackedItemId: u.r.id, locationId: u.locId,
          data: { before: beforeStr, after: afterStr, date, savedAt },
        })
      }))
      const okFlags = settled.map(s => s.status === 'fulfilled' && s.value.ok)
      const failed = units.filter((_, i) => !okFlags[i]).map(u => `${u.r.label} · ${u.locName}`)
      // 행 캡션 기준은 실제로 저장된 행만 새로 세운다.
      setRowDrafts(prev => {
        const next = { ...prev }
        units.forEach((u, i) => {
          if (!okFlags[i]) return
          const { beforeStr, afterStr } = computeRow(u.r, u.locId)
          next[locPairKey(u.r.id, u.locId)] = { savedAt, before: beforeStr, after: afterStr, itemId: u.r.id, locId: u.locId }
        })
        return next
      })
      onDraftChange?.()
      if (failed.length > 0) {
        // 실패면 칩·시각을 안 세운다 — 전부 저장된 것처럼 보이면 안 된다.
        // detail 은 라벨 하나 + 나머지 건수 — 라벨이 길면 세 개만 해도 §15 3줄을 넘긴다.
        pushToast('error', `${failed.length}건 임시저장에 실패했습니다`, {
          detail: failed.length === 1 ? failed[0] : `${failed[0]} 외 ${failed.length - 1}건`,
        })
        return
      }
      setLocDraftSavedAt(savedAt)
      locDraftSnapRef.current = snapshotOfInputs()
      pushToast('success', `${units.length}건 임시저장됨`)
    } finally {
      setDraftPending(false)
    }
  }

  // §12 3상태 칩의 기준 스냅샷 — 서브트리 전체 입력값을 한 문자열로.
  const snapshotOfInputs = () => JSON.stringify({
    locId, date,
    rows: groups.flatMap(g => g.items.map(r => ({ k: locPairKey(r.id, g.node.id), ...computeRow(r, g.node.id) }))),
  })

  // 임시저장 비우기 — 여러 행을 한 번에 지우므로 §14 확인창을 거치고, 지운 뒤 §16 적용취소를
  // 노출한다(운영자 결정 2026-09-14). 아이템별 폼(1품목)은 종전대로 즉시다 — 규모가 달라서지
  // 형제가 갈린 것이 아니다.
  const handleClearLocDrafts = async () => {
    if (!locId) return
    const targets = Object.values(rowDrafts)
    if (targets.length === 0) { setLocDraftSavedAt(null); locDraftSnapRef.current = null; return }
    const ok = await confirmDialog({
      title: `${targets.length}건의 임시저장을 비울까요?`,
      message: '입력한 숫자는 화면에 남지만 임시저장은 사라집니다. 비운 뒤 6초 안에 되돌릴 수 있습니다.',
      level: 'caution',
      confirmLabel: '비우기',
    })
    if (!ok) return
    ...
    setRowDrafts(prev => {
      const next = { ...prev }
      targets.forEach((t, i) => { if (okFlags[i]) delete next[locPairKey(t.itemId, t.locId)] })
      return next
    })
    setDraftCleanupFailed([])
    onDraftChange?.()
    if (okFlags.some(v => !v)) { pushToast('error', '임시저장 일부를 비우지 못했습니다'); return }
    setLocDraftSavedAt(null)
    locDraftSnapRef.current = null
    ...
    pushToast('success', '임시저장 비움', {
      action: { label: '적용취소', run: () => { void (async () => {
        const back = await Promise.allSettled(snapshot.map(d => saveStockCheckDraft({
          trackedItemId: d.trackedItemId, locationId: d.locId,
          data: {
            before: d.data?.before != null ? String(d.data.before) : '',
            after:  d.data?.after  != null ? String(d.data.after)  : '',
            date: restoreDate,
            savedAt: typeof d.data?.savedAt === 'number' ? d.data.savedAt : Date.now(),
          },
        })))
        const failedBack = back.filter(b => !(b.status === 'fulfilled' && b.value.ok)).length
        if (failedBack > 0) { pushToast('error', `${failedBack}건을 되돌리지 못했습니다`); onDraftChange?.(); return }
        onDraftChange?.()   // 카드 '점검 중' 배지
        // 칩·행 캡션도 다시 세운다 — 화면이 서버와 갈리면 '되돌렸다'가 말뿐이 된다.
        // 늦게 눌러 위치를 옮긴 뒤라면 이 패널 상태는 안 건드린다(다른 위치의 사실이 된다).
        if (locIdRef.current === restoreLocId) {
          const restored: Record<string, { savedAt: number; before: string; after: string; itemId: string; locId: string }> = {}
          let latest = 0
          for (const d of snapshot) {
            const savedAt = typeof d.data?.savedAt === 'number' ? d.data.savedAt : 0
            restored[locPairKey(d.trackedItemId, d.locId)] = {
              savedAt,
              before: d.data?.before != null ? String(d.data.before) : '',
              after:  d.data?.after  != null ? String(d.data.after)  : '',
              itemId: d.trackedItemId, locId: d.locId,
            }
            if (savedAt > latest) latest = savedAt
          }
          setRowDrafts(restored)
          locDraftSnapRef.current = null   // 복원본을 기준 스냅샷으로 다시 시딩(§12 3상태)
          if (latest > 0) setLocDraftSavedAt(latest)
        }
        pushToast('info', '임시저장을 되돌렸습니다')
      })() } },
    })
  }

  // ── 패널 안 옮기기 완료 (2026-09-15) ────────────────────────────────
  // 옮긴 순간 그 두 쌍(품목×출발지, 품목×도착지)의 **미저장 입력은 무효**다. 점검 시점 정렬은
  // 옮김이 점검 전이든 후든 정합인데, 깨지는 경우가 하나 있다 — '옮기기 전에 센 값을 옮긴 뒤에
  // 저장' 이다. 그 한 경우를 화면이 막는다. 메모리 입력값과 서버 임시저장을 함께 비우고, 비우기
  // 전 스냅샷을 들고 있다가 적용취소가 그대로 되쓴다(§16, handleClearLocDrafts 와 같은 문법).
  const handleTransferDone = async (res: TransferDone) => {
    const pairs = [
    ...
    // 실패 때문에 사라지면 그게 더 큰 사고다(§16 원칙). 못 비운 사실은 같은 토스트의 부가 줄로
    // 말한다(§15 장문형, §27.2 채널은 하나).
    const cleanupFailed = okFlags.some(v => !v)

    const fromName = locsRef.current.find(l => l.id === res.fromId)?.pathName ?? ''
    const toName = locsRef.current.find(l => l.id === res.toId)?.pathName ?? ''
    const movedRow = rows.find(r => r.id === res.trackedItemId)
    ...
    pushToast('success', res.swap
      ...
      action: { label: '적용취소', run: () => { void (async () => {
        ...
        const back = await Promise.allSettled(had.map(s => saveStockCheckDraft({
          trackedItemId: s.itemId, locationId: s.locId,
          data: { before: s.draft!.before, after: s.draft!.after, date: restoreDate, savedAt: s.draft!.savedAt || Date.now() },
        })))
        const backOk = back.map(b => b.status === 'fulfilled' && b.value.ok)
        ...
        onDraftChange?.()
        onDone()
        if (failedBack > 0) { pushToast('error', `임시저장 ${failedBack}건을 되돌리지 못했습니다`); return }
        pushToast('info', '옮기기를 적용취소했습니다')
      })() } },
    })
  }
      ...
      <div
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--warm-border)] shrink-0">
          <div>
            ...
            <p className="text-[0.6875rem] text-[var(--warm-muted)] mt-0.5">
              채우기 전·후를 입력하면 늘어난 만큼 그 품목의 창고(허브)에서 자동 차감됩니다. 아래 위치까지 한 번에 입력할 수 있습니다.
            </p>
          </div>
          {!inline && (
        ...
        <div className={inline ? 'px-5 py-3 space-y-3' : 'flex-1 overflow-y-auto overscroll-contain px-5 py-3 space-y-3'}>
          {!locId ? (
            ...
            <>
              ...
              {(() => {
                const times = groups.flatMap(g => g.items)
                ...
                return (
                  <div key={node.id} className="space-y-1">
                      ...
                      <SectionHeader
                        ...
                        name={
                          <span className="inline-flex items-center gap-1.5">
                            {/* 트리 행은 그 노드 한 칸의 이름이다 — 조상 경로는 들여쓰기가 말한다. */}
                            <span className={items.length === 0 ? 'font-semibold text-[var(--warm-muted)]' : undefined}>{node.name}</span>
                            {node.isHub && <Badge tone="pale-amber">기본 창고</Badge>}
                          </span>
                        }
                    ...
                    {!isCollapsed && items.map(r => {
                      ...
                      const prev = r.currentLocationBreakdown.find(lb => lb.locationId === node.id)
                      const k = locPairKey(r.id, node.id)
                      const { beforeStr, afterStr, restocked } = computeRow(r, node.id)
                      // 이 칸이 '이 품목'의 허브인지 — 품목마다 허브가 다르므로 행별로 판정.
                      ...
                      const rowDraftEdited = rowDraft != null && (beforeStr !== rowDraft.before || afterStr !== rowDraft.after)
                      return (
                        <div key={k} style={{ marginLeft: indent }} className="space-y-1 border-b border-[var(--warm-border)]/40 pb-2 last:border-0">
                          <div className="flex items-baseline justify-between gap-2">
                            <div className="min-w-0 flex-1">
                          ...
                          {/* 참고줄 — 저장된 잔량·저장된 옮김·창고 잔량. 창고 잔량만 있어도 띄운다(보충 판단의 전제).
                              '직전'이 아니라 '저장된' 이다 — 입력칸에는 임시저장본이 들어 있을 수 있어서,
                              어느 숫자가 실제로 저장된 값인지 행마다 말해야 한다(운영자 신고 2026-09-14). */}
                          ...
                          {/* 임시저장 캡션 — 입력칸 위에, 그 행에만. 어느 숫자가 임시저장본이고 어느 것이
                              저장본인지 행 단위로 가른다. 같은 파일의 '점검 중' 과 같은 정본 Badge·같은 톤이다. */}
                          {rowDraft != null && (
                            <div>
                              <Badge tone="inspect">
                                {rowDraftEdited
                          ...
                          {rowIsHub ? (
                            ...
                            <>
                              ...
                              {/* 한 줄에 둘 — 좌: 이 칸에서 다른 곳으로 옮기기, 우: 보충 없음.
        ...
        {(saveFailed.length > 0 || draftCleanupFailed.length > 0 || (saveProgress && saveProgress.stoppedAt !== '') || error) && (
          <div className="border-t border-[var(--warm-border)] px-5 py-3 shrink-0 space-y-2">
            ...
            {saveFailed.length > 0 && (
              <div className="rounded-lg px-3 py-2 space-y-1" style={{ background: 'var(--danger-bg)', border: '1px solid var(--danger-ring)' }}>
                <p className="text-xs font-semibold" style={{ color: 'var(--danger-fg)' }}>
                  {/* 임시저장이 실제로 남아 있을 때만 그렇게 말한다 — 없는데 말하면 거짓이 된다. */}
                  {saveFailed.length}건이 저장되지 않았습니다.{' '}
                  {saveFailed.some(f => rowDrafts[f.id] != null)
                    ? '임시저장은 그대로 남아 있습니다.'
                    : '입력한 값은 화면에 그대로 있습니다.'}
                </p>
                <ul className="space-y-0.5">
                  {saveFailed.map(f => (
                    <li key={f.id} className="text-[0.65625rem] leading-relaxed" style={{ color: 'var(--danger-fg)' }}>
                      {f.label} · {f.error}
                    </li>
                  ))}
                </ul>
            ...
            {draftCleanupFailed.length > 0 && (
              <div className="rounded-lg px-3 py-2" style={{ background: 'var(--warning-bg)', border: '1px solid var(--warning-ring)' }}>
                {/* 목록은 문장 끝에 둔다 — 라벨 뒤에 조사를 붙이면 이름에 따라 '세제는/세제은' 이 갈린다. */}
                <p className="text-[0.65625rem] leading-relaxed" style={{ color: 'var(--warning-fg)' }}>
                  저장됐지만 임시저장 정리에 실패한 행이 있습니다. {draftCleanupFailed.join(', ')}. 아래 칩의 비우기로 지워 주세요.
                </p>
              </div>
            )}
            {error && <p className="text-xs text-[var(--danger-fg)] bg-[var(--danger-bg)] px-3 py-2 rounded-lg">{error}</p>}
        ...
        {locId && scopeItemCount > 0 && totalRestock > 0 && (
        ...
        {/* 이중 차감 물음 — 형제 '기존에 합치기' 바와 같은 자리·같은 문법. 행마다 모달을
            N연타로 띄우지 않고 한 번만 묻는다. 대상은 §14 영향 목록 박스 문법으로 센다. */}
        {dupUnits.length > 0 && (
          <div className="border-t border-[var(--honey)]/40 bg-[var(--honey)]/10 px-5 py-3 shrink-0 space-y-2">
            <p className="text-xs font-medium text-[var(--ink)]">
            ...
            <div className="rounded-lg px-3.5 py-3" style={{ background: 'var(--danger-bg)', border: '1px solid var(--danger-ring)' }}>
              ...
              <ul className="space-y-0.5">
                {dupUnits.slice(0, 5).map(u => {
                  ...
                  return (
                    <li key={locPairKey(u.r.id, u.locId)} className="text-[12.5px] text-[var(--ink-s)]">
                      · {u.r.label} · {u.locName} 저장된 옮김{' '}
                      <span className="num font-semibold" style={{ fontFeatureSettings: "'tnum'" }}>{fmtQty(d.prevRestocked, unit)}</span>
                      {' '}· 이번 입력{' '}
                      <span className="num font-semibold" style={{ fontFeatureSettings: "'tnum'" }}>{fmtQty(d.restocked, unit)}</span>
                    </li>
                  )
                })}
                {dupUnits.length > 5 && (
        ...
        {(draftPending || locDraftSavedAt) && (() => {
          ...
          const curSnap = snapshotOfInputs()
          // 재진입으로 복원된 드래프트를 기준 스냅샷으로 시딩한다(아이템별 폼과 같은 문법).
          // 안 세우면 판정식이 영원히 거짓이라 '임시저장 후 수정됨' 이 재진입 뒤 절대 안 뜬다.
          const restoredOnly = locDraftSavedAt != null && locDraftSnapRef.current == null
          if (restoredOnly) locDraftSnapRef.current = curSnap
          ...
          return (
            <div className="px-5 pb-1 shrink-0">
              <div className="flex items-center justify-between gap-2 text-[0.65625rem] rounded-lg px-2.5 py-1.5" style={{ background: 'var(--cream-2)' }}>
                ...
                {/* 비우기 — 아이템별 폼과 같은 자리·라벨.
                    글자만으로는 27x13px 이라 §25 유사요소 확장으로 히트영역을 44px 로 넓힌다. */}
                {!draftPending && locDraftSavedAt && (
                  <button type="button" onClick={() => { void handleClearLocDrafts() }}
                    className="relative text-[var(--warm-muted)] hover:text-[var(--warm-dark)] underline shrink-0 before:absolute before:content-[''] before:-inset-x-3 before:-inset-y-[15px]">비우기</button>
                )}
              </div>
            </div>
...
// 위치 그룹 헤더의 마커 — §22 SectionHeader 의 '위치=아이콘 14px --ink-m' 슬롯.
const LocPinMarker = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"
    strokeLinecap="round" strokeLinejoin="round" className="text-[var(--warm-muted)]" aria-hidden="true">
    <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" /><circle cx="12" cy="10" r="3" />
  </svg>
)

// ── 점검 위치 선택기 — 트리 드롭다운 (2026-09-14 6단계)
//
// 네이티브 select 로는 구조를 못 그린다(들여쓰기·품목 수·기본 창고 배지). 패널은 화면 기준
// (fixed)으로 띄운다 — 모달 본문이 세로 스크롤 컨테이너라 형제로 두면 바닥에서 잘린다.
// 자리 산수는 정본 하나(usePopoverAnchor)를 쓴다.
//
...
// ── 보관 위치 관리 모달 — 트리(2026-09-14 5단계)
//
// 표시 이름이 `name`(그 노드 한 칸)인 자리는 여기와 아래 트리 행들뿐이다. 고치는 대상이 그 한
// 칸이고 조상 경로는 들여쓰기가 말한다. 그 밖의 모든 표시 자리는 pathName 이다
// (정본 lib/locationTree.flattenDfs, 그물 scripts/check-location-name-axis.mjs).
//
...
function LocationSettingsModal({ onClose, onChanged }: { onClose: () => void; onChanged?: () => void }) {
  ...
  // 행 액션 메뉴는 화면 기준(fixed)으로 띄운다 — 모달 본문이 세로 스크롤 컨테이너라 아래쪽 행에서
  // 열면 바닥에서 잘린다. 자리 산수는 정본 하나(usePopoverAnchor)를 쓴다.
  const { pos: menuPos, triggerRef: menuAnchorRef, measure: measureMenu } =
  ...
  // 순서 저장 + §16 적용취소. 되돌리기 전용 액션은 없다 — 이전 순서 배열을 같은 액션에 다시 넘긴다.
  const saveOrder = async (parentId: string | null, ids: string[]) => {
    setPending(true); setError('')
    const res = await reorderStorageLocations(parentId, ids)
    setPending(false)
  ...
  // ▲▼ — 형제 안에서만. 화면 이웃이 아니라 형제 배열에서 자리를 맞바꾼다.
  const nudge = (node: StorageLocationNode, dir: -1 | 1) => {
    ...
    setMenuId(null)
    void saveOrder(node.parentId, ids)
  }
  ...
  // (자리표가 블록 높이를 그대로 채운다), 움직이는 것은 ref 로 DOM 을 직접 쓰는 고스트 하나뿐이다.
  // state 로 올라가는 것은 표시선 자리가 **바뀔 때만**이다.
  const [drag, setDrag] = useState<LocDragView | null>(null)
  const [drop, setDrop] = useState<LocDrop | null>(null)
  ...
  const updateLocDrop = (clientY: number) => {
    ...
    if (!s || !ul) return
    const next = locHitOf(s, clientY - ul.getBoundingClientRect().top, locsRef.current)
    if (locDropEq(dropRef.current, next)) return
    dropRef.current = next
    setDrop(next)
  }

  // 가장자리 자동 스크롤 — 모달 본문 위·아래 48px 감지대, 진입 깊이에 비례해 초당 0~420px.
  const runLocAutoScroll = () => {
    if (rafRef.current != null) return
    const step = (t: number) => {
      const s = dragRef.current
      if (!s) { rafRef.current = null; return }
      const dt = s.lastTick === 0 ? 16 : Math.min(64, t - s.lastTick)
      s.lastTick = t
      const box = s.scroller?.getBoundingClientRect()
      if (box && s.scroller) {
        const over = s.pointerY - (box.bottom - LOC_SCROLL_BAND)
        const under = (box.top + LOC_SCROLL_BAND) - s.pointerY
        const v = over > 0 ? Math.min(1, over / LOC_SCROLL_BAND) * LOC_SCROLL_MAX
          : under > 0 ? -Math.min(1, under / LOC_SCROLL_BAND) * LOC_SCROLL_MAX : 0
        if (v !== 0) {
          const before = s.scroller.scrollTop
          s.scroller.scrollTop = before + (v * dt) / 1000
          if (s.scroller.scrollTop !== before) updateLocDrop(s.pointerY)
        }
      }
  ...
  const onLocHandleDown = (id: string) => (e: React.PointerEvent) => {
    ...
    // 행 rect 는 여기서 **한 번만** 잰다. 자리표가 블록 높이를 그대로 채우므로 드래그 중 남은
    // 행들은 제자리고, 스크롤은 목록 rect 하나만 다시 읽어 보정한다.
    const rects = lis.map(li => li.getBoundingClientRect())
    const ctx = locDropCtx(all, node)
    const block = locSubtree(all, id)
    const first = all.findIndex(n => n.id === id)
    const rows: LocHitRow[] = []
    for (let i = 0; i < all.length; i++) {
      if (ctx.blocked.has(all[i].id)) continue
      // 그 행 서브트리의 **보이는** 마지막 바닥 — 몸통 드롭('자식 맨 뒤')의 표시선 자리다.
      let last = i
      for (let j = i + 1; j < all.length && all[j].depth > all[i].depth; j++) if (!ctx.blocked.has(all[j].id)) last = j
      rows.push({
        id: all[i].id, depth: all[i].depth,
        top: rects[i].top - listRect.top,
        bottom: rects[i].bottom - listRect.top,
        subtreeBottom: rects[last].bottom - listRect.top,
      })
    }
    dragRef.current = {
      ctx, rows,
      blockTop: rects[first].top - listRect.top,
      blockBottom: rects[first + block.length - 1].bottom - listRect.top,
      grabDy: e.clientY - rects[first].top,
      scroller: ul.closest('.overflow-y-auto') as HTMLElement | null,
      pointerY: e.clientY, lastTick: 0,
    }
    ...
    setDrag({
      ...
      left: rects[first].left, width: rects[first].width, y0: rects[first].top,
    })
    runLocAutoScroll()
  }
  ...
  const onLocHandleMove = (e: React.PointerEvent) => {
    ...
    // 고스트는 state 를 안 탄다 — transform 한 줄을 ref 로 DOM 에 직접 쓴다.
    if (ghostRef.current) ghostRef.current.style.transform = `translate3d(0, ${e.clientY - s.grabDy}px, 0)`
    updateLocDrop(e.clientY)
  }

  // 놓는다. 고스트는 **자리표 위로** 앉는다(150ms) — 낙관 반영이 없으니 성공이든 실패든 같은 자리다.
  const onLocHandleUp = () => {
    const s = dragRef.current
    const d = dropRef.current
    if (!s) return
    stopLocAutoScroll()
    const dragNode = s.ctx.node
    const g = ghostRef.current
    const ul = locListRef.current
    if (g && ul) {
      g.style.transition = 'transform 150ms var(--ease-sharp)'
      g.style.transform = `translate3d(0, ${ul.getBoundingClientRect().top + s.blockTop}px, 0)`
    }
    ...
    void commitLocDrop(d, dragNode)
  }

  // 취소(시스템이 포인터를 거둬감)는 **아무것도 저장하지 않는다**. 드롭과 같은 길로 보내면
  // 전화가 오거나 제스처가 가로채인 것만으로 위치가 옮겨진다.
  const onLocHandleCancel = () => {
  ...
  const commitLocDrop = async (d: LocDrop, node: StorageLocationNode) => {
    setPlacing(true)
    try {
      // 같은 부모 안에서 자리만 바뀐 드롭은 순서 저장 정본이 맡는다 — 부모를 안 건드리니 이름 제안도
      // 세 걸음 적용취소도 필요 없고, §16 은 그 액션에 이미 들어 있다.
      if (d.parentId === node.parentId) {
        const ids = locsRef.current.filter(n => n.parentId === d.parentId).map(n => n.id)
        const from = ids.indexOf(node.id)
        if (from < 0) return
        ids.splice(from, 1)
        ids.splice(d.index, 0, node.id)
        await saveOrder(d.parentId, ids)
        return
      }
      setError('')
      const res = await placeStorageLocation(node.id, d.parentId, d.index, locPreserveDefault(d.parentId))
      if (!res.ok) { setError(res.error); reload(); return }
  ...
  const handleAddChild = async (parentId: string) => {
    ...
    setPending(true); setError('')
    const res = await createStorageLocation(childName, parentId)
    setPending(false)
    if (!res.ok) { setError(res.error); return }
    setChildName(''); setChildOf(null); reload()
  }

  // 이름 바꾸기 — 서버가 이전 이름을 undo 로 돌려주므로 같은 액션을 다시 불러 되돌린다(§16).
  ...
  // 다시 읽어 준다.
  const onMoved = (undo: LocationMoveUndo, renameLabel?: string, title?: string) => {
  ...
  // 하위가 있는 위치는 force 로도 서버가 거부한다 — 그 문구를 **인라인 그대로** 보여준다(§27.2).
  ...
  const menuNode = menuId ? locs.find(n => n.id === menuId) ?? null : null
  ...
  // footer 캡션 자리에 평시 문장 대신 들어갈 말. null 이면 평시 문장이 그대로 보인다.
    ...
    <Modal open onClose={onClose} title="보관 위치 관리" subtitle="창고 · 4층 주방 같은 보관 장소를 등록하고, 냉장고 상단처럼 아래 칸을 둘 수 있습니다" width="sm"
      ...
      footer={
        <div className="space-y-2">
          ...
          {/* 캡션 상자는 **하나다**(재검수 2026-09-16). 막힌 이유·깊이 안내·저장 중을 여기서 함께
              ...
              **높이는 글자가 아니라 평시 문장이 정한다**(디자이너 검수 2026-09-16). 평시 문장은
              320px 에서 세 줄인데 갈아 끼우는 이유·저장 중 문구는 전부 한 줄이다. 게다가 `reasonOf`
              가 제자리에서 곧바로 `지금 있는 자리입니다` 를 돌려주고 첫 onLocHandleMove 에 이동 임계가
              없어서, **1px 만 움직여도 세 줄이 한 줄로 주저앉는다.** 그 순간 오버레이(items-center)가
              패널을 재정렬하고, pointerdown 에 캐시한 좌표로 도는 고스트는 자리표와 어긋난다.
      ...
      <div className="px-5 sm:px-6 py-4 space-y-4">
        ...
        {/* 표시선은 목록 위에 겹쳐 뜬다 — 흐름에 끼워 넣으면 그 2px 만큼 아래 행이 밀려 내려가
            드래그 내내 목록이 떨린다(자리표가 높이를 지키는 이유와 같다). */}
        <div className="relative">
        <ul ref={locListRef} className="space-y-1.5">
          {locs.map(node => drag != null && drag.ids.has(node.id) ? (
            // 자리표 — 들린 블록 **전체 높이 그대로** 한 칸. 남은 행은 한 픽셀도 안 움직인다.
            node.id === drag.id ? (
              <li key={node.id} aria-hidden
                style={{ marginLeft: (node.depth - 1) * LOC_INDENT_PX, height: drag.height }}
                className="border border-dashed border-[var(--warm-border)] rounded-xl bg-transparent" />
            ) : null
          ) : (
            <li key={node.id}
              ...
              {editId === node.id ? (
                ...
                <>
                  {/* 왼쪽 44pt 드래그 손잡이 — 순서와 상위 위치를 함께 바꾼다. 포인터가 없는
                      조건에서도 트리를 고칠 수 있어야 하므로 관리 메뉴의 옮기기·위로·아래로가 정본
                      입구고 이 손잡이는 보조다. 설정 옵션 리스트와 같은 문법이다. */}
                  <button type="button" aria-label={`${node.name} 순서·상위 위치 이동`}
                    onPointerDown={onLocHandleDown(node.id)} onPointerMove={onLocHandleMove} onPointerUp={onLocHandleUp} onPointerCancel={onLocHandleCancel}
                    style={{ touchAction: 'none' }}
                  ...
                  <span className="flex-1 min-w-0 truncate text-sm text-[var(--warm-dark)]">{node.name}</span>
                  {node.isHub && <Badge tone="pale-amber" className="shrink-0">기본 창고</Badge>}
                  {/* 메뉴가 뜰 자리의 기준점 — RowActionBtn 은 히트영역과 크롬을 나눈 정본이라
                      이벤트를 내주지 않는다. 행마다 감싼 span 을 앵커로 쓴다. */}
                  <span className="shrink-0" ref={el => { if (el) rowAnchors.current.set(node.id, el); else rowAnchors.current.delete(node.id) }}>
                    <RowActionBtn
                      onClick={() => {
                        if (menuId === node.id) { setMenuId(null); return }
                        menuAnchorRef.current = rowAnchors.current.get(node.id) ?? null
                        measureMenu()
                        setMenuId(node.id)
                      }}>
                      관리
        ...
        {childOf && (
          <div className="flex gap-2 items-center" style={{ marginLeft: ((locs.find(n => n.id === childOf)?.depth ?? 1)) * LOC_INDENT_PX }}>
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter') handleAddChild(childOf); if (e.key === 'Escape') { setChildOf(null); setChildName('') } }}
              placeholder={`${locs.find(n => n.id === childOf)?.name ?? ''} 아래에 만들 이름 (예: 상단)`}
              className="flex-1 min-w-0 bg-[var(--canvas)] border border-[var(--warm-border)] rounded-sm px-3 py-2.5 text-sm text-[var(--warm-dark)] outline-none focus:border-[var(--coral)]" />
            <Btn variant="primary" size="sm" disabled={pending || !childName.trim()} onClick={() => handleAddChild(childOf)}>추가</Btn>
            <Btn variant="ghost" size="sm" onClick={() => { setChildOf(null); setChildName('') }}>취소</Btn>
          </div>
        )}
      ...
      {/* 드래그 고스트 — body 로 뺀다. 모달 본문 안에 두면 등장 애니메이션의 transform 이 남아
          있는 동안 position:fixed 의 기준이 뷰포트가 아니라 그 패널이 되어 한 프레임 튄다.
...
// ── 위치 옮기기 모달 (2026-09-14 5단계)
// 목적지를 고르는 자리다. 자기 서브트리·깊이 초과는 **서버 거부에 기대지 않고** 여기서 비활성 +
// 이유 캡션으로 먼저 말한다. 눌러 보고 나서야 왜 안 되는지 알게 하면 그것이 곧 오조작이다.
// 판정은 서버와 같은 순수 함수(lib/locationTree)를 쓴다 — 사본을 만들면 언젠가 갈린다.
function LocationMoveModal({ all, node, onClose, onDone }: {
  all: StorageLocationNode[]
  node: StorageLocationNode
  onClose: () => void
  onDone: (undo: LocationMoveUndo, renameLabel?: string, title?: string) => void
}) {
  // 기본 목적지는 '고르지 않음'이다 — 최상위(null)를 기본으로 두면 한 번 잘못 눌러 루트로 튄다.
  const [dest, setDest] = useState<{ id: string | null } | null>(null)
  // 이름 제안 기본값은 **방향별**이다(운영자 결정 2026-09-16, 드래그와 같은 규칙) — 부모가 남으면
  // 켜고 최상위로 빼면 끈다. 손으로 준 값은 목적지를 바꾸는 순간 놓아 준다. 방향이 바뀌었는데
  // 앞 목적지에서 준 값이 따라오면, 고른 자리의 기본값과 체크 상태가 말없이 어긋난다.
  const [stripOverride, setStripOverride] = useState<boolean | null>(null)
  const chooseDest = (id: string | null) => { setDest({ id }); setStripOverride(null) }
  const [pending, setPending] = useState(false)
  const [error, setError] = useState('')

  // 판정 밑감 한 벌 — 드래그와 **같은 reasonOf** 를 부른다(사본 금지).
  const ctx = locDropCtx(all, node)
  ...
  const nextName = dest === null ? node.name : nameFor(chosen)
  const stripPreview = nextName !== node.name ? `${node.name} → ${nextName}` : null
  // 라벨은 방향이 말한다 — 줄면 떼기, 늘면 붙이기(제안이 없으면 떼기 쪽 중립).
  const stripLabel = locPreserveLabel(node.name, nextName)
  const chosenReason = dest === null ? null : reasonFor(chosen)

  const submit = async () => {
    if (dest === null) return
    setPending(true); setError('')
    const res = await moveStorageLocation(node.id, dest.id, strip)
    setPending(false)
    if (!res.ok) { setError(res.error); return }
  ...
  return (
    <Modal open onClose={onClose} title={`'${node.pathName}' 옮기기`} subtitle="옮길 자리를 고르세요. 옮긴 뒤 6초 안에 적용취소할 수 있습니다" width="sm" z={260}>
      <div className="space-y-3">
        ...
        {/* 이름 제안 — §29 값의 전환 표기(화살표는 이 한 자리에만 쓴다). 라벨은 방향이 말한다.
        ...
        <label className="flex items-center gap-2.5 min-h-[44px] cursor-pointer">
          <input type="checkbox" checked={strip} onChange={e => setStripOverride(e.target.checked)}
            className="w-4 h-4 accent-[var(--coral)] shrink-0" />
          <span className="text-sm text-[var(--warm-dark)] shrink-0">{stripLabel}</span>
          {stripPreview
        ...
        {/* 체크박스를 켜 이름이 그 자리의 형제와 겹치게 됐을 때 — 눌러 보고 서버 거부를 읽게
            하지 않는다(목적지 행의 회색 처리와 같은 규칙, 같은 문구). */}
        {chosenReason && <p className="text-[0.65625rem] text-[var(--warm-muted)]">{chosenReason.text}</p>}
        <ModalFooterActions onCancel={onClose}>
          <Btn variant="primary" disabled={pending || dest === null || chosenReason != null} onClick={() => { void submit() }}>
            {pending ? '옮기는 중…' : '옮기기'}
          </Btn>
        </ModalFooterActions>
      </div>
