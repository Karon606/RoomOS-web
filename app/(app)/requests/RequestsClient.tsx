...
import {
  resolveTenantRequest, deleteTenantRequest,
  ...
  const handleResolve = (id: string, memo: string) => {
    startTransition(async () => {
      ...
      try {
        ...
        setResolvingId(null); setResolvingMemo('')
        router.refresh()
      } finally { release() }
    })
  }
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(['unresolved', 'all', 'resolved'] as const).map(s => (
          <button key={s} onClick={() => setFilterStatus(s)}
            ...
            {s === 'unresolved' ? '미처리' : s === 'resolved' ? '처리됨' : '전체'}
          </button>
        ))}
        ...
        <button onClick={() => setFilterCategory('all')}
          className={`px-3 py-1.5 text-xs font-medium rounded-full transition-colors ${
            filterCategory === 'all' ? 'bg-[var(--ink-2)] text-white' : 'bg-[var(--cream)] text-[var(--warm-mid)] border border-[var(--warm-border)]'
          }`}>
          전체 카테고리
        </button>
        {CATEGORIES.map(cat => {
          ...
          return (
            <button key={cat} onClick={() => setFilterCategory(cat)}
              className={`px-3 py-1.5 text-xs font-medium rounded-full transition-colors ring-1 ${
                active ? `${c.bg} ${c.fg} ${c.ring} ring-2` : `bg-[var(--cream)] text-[var(--warm-mid)] ring-[var(--warm-border)]`
              }`}>
              {cat}
            </button>
          )
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="space-y-2">
          {filtered.map(r => {
            ...
            return (
              <li key={r.id} className={`rounded-2xl p-4 ${isResolved ? 'opacity-60 bg-[var(--canvas)]' : 'bg-[var(--cream)]'} border border-[var(--warm-border)]`}>
                {/* 메타 */}
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  {r.isUrgent && !isResolved && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-red-50 text-red-600 ring-1 ring-red-200">🚨 긴급</span>
                  )}
                  {c && (
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${c.bg} ${c.fg} ring-1 ${c.ring}`}>
                      {r.category}
                    </span>
                  )}
                  {isResolved && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200">완료</span>
                  )}
                  <Link href={`/tenants?tenantId=${r.tenantId}&tab=requests`}
                ...
                {/* 처리 메모 */}
                {r.resolutionMemo && (
                  <p className="text-xs text-emerald-700 mt-2 pt-2 border-t border-emerald-200/40">↳ {r.resolutionMemo}</p>
                )}
                  ...
                  resolvingId === r.id ? (
                    <div className="mt-3 space-y-2 rounded-lg p-2.5 bg-emerald-50/40 border border-emerald-200/50">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => { setResolvingId(null); setResolvingMemo('') }} disabled={pending}
                        ...
                        <button onClick={() => handleResolve(r.id, resolvingMemo)} disabled={pending}
                    ...
                    <div className="flex gap-2 mt-3">
                      <button onClick={() => { setResolvingId(r.id); setResolvingMemo('') }} disabled={pending}
