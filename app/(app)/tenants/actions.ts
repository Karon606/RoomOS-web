 ...
 * 국적을 대한민국으로 바꿔 칸이 사라진 경우도 같은 규칙이라 기존 값이 보존된다(해외 연락처 관행).
 */
...
export async function addTenant(formData: FormData): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  //
  // 연락처 조립을 이 문 **위**로 올린 이유는 예약 확정 문 하나다(2026-09-16). 새 입주자는 아직
...
export async function updateTenant(formData: FormData): Promise<
  | {
      ...
      shortSync?: { leaseTermId: string; diff: number; newRent: number; kind: 'increase' | 'decrease' }
      /**
...
> {
  ...
  /**
   * 예약 확정 문 — 본인 전화번호가 없으면 방을 잡아 두지 않는다(운영자 결정 2026-09-16).
  ...
  try {
    ...
    const lease = await prisma.leaseTerm.findUnique({
      ...
      select: {
        ...
        rentAmount: true, moveInDate: true, dueDay: true, roomSchedule: true,
    ...
    // **계획이 있으면 오늘 들어갈 방으로 본다** — 계약 호실이 아직 차 있어도 오늘은 임시 방으로
    // 들어가는 것이라 막을 이유가 없다(미리 잡아 둔 일정의 그날이 이 자리다).
    const planned = scheduledSegmentOn(parseRoomSchedule(lease.roomSchedule), kstYmdStr())
    const blocked = await moveInBlock(planned?.roomId ?? lease.roomId, leaseTermId)
 ...
 * 안 잡혀 있으면 **모른다**(null) — 그때는 일정을 짤 수 없고 화면이 그 사실을 말한다.
 */
...
async function reservationRoomBusy(propertyId: string, leaseTermId: string): Promise<{
  ...
  const lease = await prisma.leaseTerm.findFirst({
    where: { id: leaseTermId, propertyId },
    select: {
      id: true, status: true, roomId: true, moveInDate: true, reservationConfirmedAt: true,
      roomSchedule: true, room: { select: { roomNo: true } },
    },
  })
  if (!lease?.roomId || !lease.moveInDate) return null
  if (lease.status !== 'RESERVED' || !lease.reservationConfirmedAt) return null
  // 이미 일정을 잡아 뒀으면 물을 것이 없다.
...
/**
 * 일정에 적힌 방이 그 기간에 실제로 비는가 — 어긋나면 사람이 읽을 사유, 맞으면 null.
 *
 * 세 축을 다 본다. 실제 점유 구간, 아직 구간이 없는 예약(입주 희망일부터 무기한),
 * **그리고 남이 미리 잡아 둔 일정**. 셋째를 빼면 402호를 두 사람이 같은 날 예약해 두고
 * 그날 둘 다 들어가는 길이 열린다.
 *
 * 미리 잡기와 실제 처리가 같은 함수를 쓴다 — 두 벌이면 한쪽만 느슨해진다.
 */
async function roomScheduleClash(
  propertyId: string,
...
/**
 * 호실 일정 화면이 쓸 재료 — 어느 날부터 채워야 하고, 그 기간에 쓸 수 있는 방이 무엇인가.
 *
 * 후보 판정에 기존 '입주 가능'(roomAvailability)을 안 쓴다. 그것은 하루 축이라 **뒤에 무기한
 * 예약이 걸린 방을 통째로 뺀다** — 실측(2026-08-26)에서 402·409 둘 다 9/8 예약 때문에 빠졌다.
 * 여기서 묻는 것은 "언제부터 언제까지 쓸 수 있나"라 구간으로 답해야 한다.
 *
 * **예약(RESERVED)도 센다.** 예약은 점유 구간이 없어서(lib/roomStay 자격 게이트) 구간만 보면
 * 아직 입주 안 한 예약자가 들어올 방이 비어 보인다. 그 방을 골라 두면 그 사람이 왔을 때 겹친다.
 */
export async function getRoomScheduleOptions(leaseTermId: string, fromYmd: string): Promise<{
  ok: true
  mainRoomNo: string
  mainRoomId: string
  moveInDate: string
  /**
   * 계약 호실을 쓸 수 있게 되는 날 — 일정의 끝점이다. 그날 이 사람이 본 방으로 든다.
   * null 이면 앞사람 퇴실 예정일이 안 정해져 언제 빌지 모르는 것이다(일정을 못 짠다).
   */
  ...
  try {
    ...
    const lease = await prisma.leaseTerm.findFirst({
      where: { id: leaseTermId, propertyId },
      select: { id: true, roomId: true, moveInDate: true, room: { select: { roomNo: true } } },
    })
    ...
    const [rooms, spans, mainOccupants] = await Promise.all([
      prisma.room.findMany({
        ...
        where: { propertyId, id: { not: lease.roomId }, NOT: vacancyExcludedWhere },
    ...
    return {
      ...
      rooms: rooms
        ...
        .filter(r => r.availableUntil !== fromYmd),
    }
  } catch (err) {
    if ((err as { digest?: string })?.digest?.startsWith('NEXT_REDIRECT')) throw err
...
/**
 * 호실 일정으로 입실 처리 — 입주일을 확정하고 일정을 저장한 뒤 첫 구간을 연다.
 *
 * **이것은 '조기 입실'이 아니다.** 예약 입주일이 바뀐 것뿐이고(운영자 정정 2026-08-26,
 * "예약을 9/1로 했으나 상황에 따라 예약일정을 충분히 바꿀 수 있는거니까"), 청구는 그 날짜에서
 * 저절로 따라온다 — 8/31 입주면 8월분이 8/31에 청구되고 다음 납부일이 9/30이다. 하루치를 따로
 * 받거나 청구를 손보는 일이 없다. 주기가 통째로 하루 앞당겨질 뿐이다.
 *
 * 그래서 이 액션이 하는 일은 셋뿐이다. 입주일 확정, 일정 저장, 첫 방의 구간 열기.
 * **이후 이동은 자가 치유(lib/roomStay)가 일정을 읽어 알아서 한다** — 운영자가 그날 무엇을
 * 누를 필요가 없다("기록 갱신도 필요없지").
 */
export async function startLeaseWithRoomSchedule(input: {
  leaseTermId: string
  /** 확정 입주일 'YYYY-MM-DD'. 예약 때 잡아 둔 날과 달라도 된다. */
  moveInDate: string
  /** 방 순서 — 마지막 줄이 계약 호실이고 기한이 없다. */
  ...
  try {
    ...
    const lease = await prisma.leaseTerm.findFirst({
      ...
      select: { id: true, status: true, tenantId: true, roomId: true, dueDay: true },
    })
    if (!lease) return { ok: false, error: '계약 정보를 찾을 수 없습니다.' }
    if (!lease.roomId) return { ok: false, error: '확정된 호실 정보가 없습니다.' }
    if (lease.status !== 'RESERVED') return { ok: false, error: '예약 상태에서만 이 처리를 할 수 있습니다.' }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(input.moveInDate)) return { ok: false, error: '입주일 형식이 올바르지 않습니다.' }
    // 미래 시작 구간은 감지망이 사고로 부른다 — 점유 이력은 실제로 있었던 일만 담는다.
    if (input.moveInDate > kstYmdStr()) return { ok: false, error: '입주일은 오늘보다 뒤로 잡을 수 없습니다.' }
    ...
    // 일정에 적힌 방이 그 기간에 실제로 비는가 — 화면이 이미 걸렀지만 서버가 다시 본다.
    ...
    const derivedDueDay = lease.dueDay == null ? dueDayFromMoveIn(moveInAt) : null
    ...
    const next = nextRoomMove(schedule, input.moveInDate)
    const nextRoom = next
      ? await prisma.room.findUnique({ where: { id: next.roomId }, select: { roomNo: true } })
      : null
    return {
      ok: true,
...
/**
 * 호실 일정을 **미리** 잡아 둔다 — 예약 상태 그대로, 점유 구간은 만들지 않는다.
 *
 * 왜 필요한가. 계약서는 입주 전에 서명받는다. 일정을 입주 당일에만 정할 수 있으면 그 전에 뽑은
 * 계약서에 호실 일정이 안 찍히고, "옮길 곳을 계약서에 다 적어둔다"는 전제가 무너진다
 * (운영자 지적 2026-08-26).
 *
 * **미래 입주일을 받는다.** 실제 처리(startLeaseWithRoomSchedule)와 갈리는 유일한 지점이다.
 * 점유 이력은 실제로 있었던 일만 담아야 하므로 여기서는 RoomStay 를 한 줄도 만들지 않는다 —
...
/** 미리 잡아 둔 일정을 지운다 — 계약은 예약 상태 그대로다. */
export async function clearRoomSchedulePlan(leaseTermId: string): Promise<{ ok: true } | { ok: false; error: string }> {
  try {
    await requireEdit()
    const { propertyId } = await getPropertyId()
    const lease = await prisma.leaseTerm.findFirst({
    ...
    if (!lease) return { ok: false, error: '계약 정보를 찾을 수 없습니다.' }
    if (lease.status !== 'RESERVED') return { ok: false, error: '예약 상태에서만 지울 수 있습니다.' }
    await prisma.leaseTerm.update({ where: { id: lease.id }, data: { roomSchedule: Prisma.DbNull } })
    revalidatePath('/tenants'); revalidatePath('/rooms'); revalidatePath('/room-manage')
    return { ok: true }
  } catch (e) {
    if ((e as { digest?: string })?.digest?.startsWith('NEXT_REDIRECT')) throw e
    return { ok: false, error: (e as Error).message ?? '지우지 못했습니다.' }
  }
}

/**
 * 일정의 다음 방으로 옮긴다 — 홈 알림의 "옮기겠습니까"가 확인되면 여기로 온다.
 *
 * **자동으로 옮기지 않는 이유.** 방을 옮기는 것은 실제로 짐을 나르는 일이라 하루 밀릴 수 있다.
 * 앱이 일정만 보고 옮기면 기록은 9/1인데 사람은 9/2에 옮긴 상태가 된다. 물어보면 진짜 옮긴
 * 날이 남는다(운영자 확정 2026-08-26).
 *
...
export async function advanceRoomSchedule(input: {
  ...
  try {
    ...
    const lease = await prisma.leaseTerm.findFirst({
      where: { id: input.leaseTermId, propertyId },
      select: {
        id: true, roomId: true, moveInDate: true, moveOutDate: true, roomSchedule: true,
        roomStays: { where: { endDate: null }, select: { roomId: true }, take: 1 },
      },
    })
    if (!lease?.roomId) return { ok: false, error: '계약 정보를 찾을 수 없습니다.' }
    const schedule = parseRoomSchedule(lease.roomSchedule)
    if (!hasRoomSchedule(schedule)) return { ok: false, error: '호실 일정이 없습니다.' }

    const fromRoomId = lease.roomStays[0]?.roomId ?? null
    if (!fromRoomId) return { ok: false, error: '지금 머무는 방을 찾을 수 없습니다.' }
    // 일정에서 지금 방 다음에 오는 줄이 목적지다 — '오늘의 방'이 아니라 순서를 따른다.
    const idx = schedule.findIndex(e => e.roomId === fromRoomId)
    if (idx < 0) return { ok: false, error: '지금 머무는 방이 일정에 없습니다.' }
    const next = schedule[idx + 1]
    if (!next) return { ok: false, error: '이미 마지막 방입니다.' }
    ...
    if ((e as { digest?: string })?.digest?.startsWith('NEXT_REDIRECT')) throw e
    return { ok: false, error: (e as Error).message ?? '옮기지 못했습니다.' }
  }
}
...
/** 호실 일정 적용취소 — 예약 상태로 되돌리고 일정과 구간을 걷는다. */
export async function undoRoomSchedule(leaseTermId: string): Promise<{ ok: true } | { ok: false; error: string }> {
  try {
    await requireEdit()
    const { propertyId } = await getPropertyId()
    const lease = await prisma.leaseTerm.findFirst({
      where: { id: leaseTermId, propertyId },
      select: { id: true, roomSchedule: true },
    })
    if (!lease) return { ok: false, error: '계약 정보를 찾을 수 없습니다.' }
    const schedule = parseRoomSchedule(lease.roomSchedule)
    if (!hasRoomSchedule(schedule)) return { ok: false, error: '호실 일정이 없습니다.' }

    const roomIds = [...new Set(schedule.map(e => e.roomId))]
    await prisma.$transaction(async tx => {
...
export async function applyStatusTransition(input: {
  ...
  try {
    ...
    if (lease.status === 'RESERVED' && input.toStatus === 'ACTIVE' && lease.roomId) {
      const plan = parseRoomSchedule(lease.roomSchedule)
      // 미리 잡아 둔 일정이 있으면 입주일이 그 첫날과 같아야 한다. 갈리면 일정의 나머지가
      // 통째로 어긋나므로(빈틈·겹침) 조용히 저장하지 않고 되묻는다.
      if (hasRoomSchedule(plan)) {
      ...
      const blocked = await moveInBlock(planned?.roomId ?? lease.roomId, input.leaseTermId)
      if (blocked) return { ok: false, error: blocked.error, code: 'ROOM_OCCUPIED' }
    }
 ...
 *
 * **화면에서는 더 이상 안 쓴다**(2026-08-31). 종전에는 퇴실 경로 셋 중 입주자 관리 수정에만
 * 이용료 환불이 있어서, 홈 알림과 프리즘이 "확정해 둔 환불이 남았다"고 말이라도 하려고 이 함수를
 * 불렀다. 지금은 세 화면이 정산 정본(components/checkout/RentSettlementSection)을 함께 써서
 * 각자 계산하고 확정하므로 그 경고가 필요 없다.
 *
 * 남겨 두는 이유는 이것이 **미확정 환불을 찾는 유일한 질의**이기 때문이다. 점검 스크립트나 앞으로
 * 생길 알림이 같은 판정을 손으로 다시 쓰면 그 순간 갈린다.
 *
