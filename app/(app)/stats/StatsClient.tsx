      ...
      <div className="rounded-xl p-5" style={{ background: 'var(--cream)', border: '1px solid var(--warm-border)' }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={handleAnalyze} disabled={isPending}
            ...
            {isPending
              ...
              : <><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2l1.8 8.2L22 12l-8.2 1.8L12 22l-1.8-8.2L2 12l8.2-1.8z" /></svg>AI 분석하기</>}
          </button>
        </div>
        ...
        {aiText && !isPending && (
          <div className="rounded-xl p-4" style={{ background: 'var(--coral-pale)', border: '1px solid color-mix(in srgb, var(--coral) 20%, transparent)' }}>
            ...
            <button onClick={handleAnalyze} className="mt-3 text-xs inline-flex items-center gap-1" style={{ color: 'var(--coral)' }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M21 12a9 9 0 1 1-2.64-6.36M21 3v6h-6" /></svg>다시 분석</button>
          </div>
        )}
      </div>
    </div>
