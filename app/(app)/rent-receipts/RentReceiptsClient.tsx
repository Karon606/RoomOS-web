            ...
            {fileRows.map(c => {
              ...
              <li key={c.id}
                ...
                <div className="flex items-center gap-1.5 flex-wrap sm:shrink-0 sm:justify-end">
                  ...
                  <SendDocButton getPdfBytes={fetchDocBytes(c.driveFileId)} fileName={`${c.tenantName}_${docFileLabel(isDeposit ? 'deposit' : 'rent', c.nameStyle ?? 'ko')}`}
                  ...
                  <PrintDocButton driveFileId={c.driveFileId} from="rent-receipts"
                    fileName={`${c.tenantName}_${docFileLabel(c.kind === 'deposit' ? 'deposit' : 'rent', c.nameStyle ?? 'ko')}`} />
