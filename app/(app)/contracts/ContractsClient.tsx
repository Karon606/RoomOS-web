          ...
          {rows.map(c => {
            ...
            <li key={c.id}
              ...
              <div className="flex items-center gap-1.5 flex-wrap sm:shrink-0 sm:justify-end">
                ...
                {!c.voidedAt && (
                  <SendDocButton getPdfBytes={fetchDocBytes(c.driveFileId)} fileName={`${c.docName}_${docFileLabel('contract', c.nameStyle ?? 'ko')}`}
                    className={btnClass('secondary', 'sm')} />
                )}
                {/* 인쇄 = §30 이 등재한 여섯 번째 동사. 종전에는 뷰어 안에만 있었다(신고 71753b36).
                    계약서는 여러 장이라 뷰어를 거치는 비용이 특히 컸다. */}
                {!c.voidedAt && (
                  <PrintDocButton driveFileId={c.driveFileId} fileName={`${c.docName}_${docFileLabel('contract', c.nameStyle ?? 'ko')}`} from="contracts" />
                )}
