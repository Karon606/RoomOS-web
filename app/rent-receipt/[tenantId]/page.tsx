...
export default async function RentReceiptPage({
  ...
  searchParams: Promise<{ month?: string; kind?: string; leaseTermId?: string }>
}) {
  const { tenantId } = await params
  const { month, kind, leaseTermId } = await searchParams
  ...
  return <RentReceiptView key={`${receiptKind}-${data.anchorMonth}-${data.leaseTermId ?? ''}`} data={data} />
}
