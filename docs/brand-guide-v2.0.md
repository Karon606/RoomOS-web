...
## §12 폼 입력
- iOS 소프트 키보드는 layout viewport(dvh)를 줄이지 않는다. 셸 안 긴 폼은 하단 입력칸이 키보드 뒤에 깔린다. 셸 본문은 겹침만큼 padding-bottom 을 더해 스크롤 여유를 만들고(높이를 줄이면 --shell-content-h 를 쓰는 편집기류까지 파급된다), 포커스된 칸이 가려졌으면 그만큼만 컨테이너를 스크롤한다. 모달은 --modal-vvh 로 max-height 를 줄인다. 키보드 감지는 단일 구독처(ViewportOffsetGuard)에서만 한다.
