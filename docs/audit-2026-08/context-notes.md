...
## 2026-08-01 · 운영자 결정 (A페이즈 관련)

- **김민정 172,000원 현금영수증**: "카드결제했기 때문에 자동 발행이겠지."
  → 카드 매출전표가 증빙을 대신한다는 뜻. 기록을 지우지 말고 **집계 이중 계상만 막는다**
  (현금영수증 합계와 카드 수납 합계에 동시에 잡히던 것). 앞으로는 입력 시 카드 계열이면 안내한다.
- **보증금 미반환분(H 항목)**: "보증금 수납기록이 없으면 기타수익으로."
  → 실수납 기록이 없어도 미반환분은 기타수익으로 인식하는 현행 유지. **H 는 수정 대상 아님.**
- **월 마감 기능**: "남겨놨다가 나중에 꼭 하자." → 백로그. 이번 A페이즈 범위 밖.
- 진행 순서: 재고 창고 표시 → A페이즈 1단계 → 이후 차례대로.
