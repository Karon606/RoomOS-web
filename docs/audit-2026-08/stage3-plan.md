...
- [x] 구현 완료 (2026-08-02, 한 커밋)
- [x] 검증 1~4 통과

### 최종 범위 조정 — 무청구 게이트(#12)는 건드리지 않았다

새 규칙 `기간월 < mon <= 퇴실월` 과 현재 표현이 **모든 도달 가능한 경우에서 같은 답**을 낸다.
정산 기간은 최대 두 달에 걸치므로 기간월과 퇴실월 사이에 중간 달이 없기 때문이다.
바꿔서 얻는 것은 임시조정 일관성뿐인데 해당 계약이 0건이고, 대신 359만원이 걸린 여섯 화면을
건드리게 된다. 남는 장사가 아니라 그대로 뒀다. 주석만 갱신했다.

### 검증 결과

1. 합성값 979행(2026-02~12, 게이트 포함) 전후 대조 — **차이 0행.** 기존 데이터 안 움직임
2. 새 규칙이 실제로 다른 답을 내는지 — **12건이 올바른 기간으로 귀속.** 예: 502호 남태우
   퇴실월 2026-07 에서 정산월 2026-06(기간 6/26~7/25, 29일)
3. 회귀 주입 — 퇴실월 기준으로 되돌리면 1건, 입주월 보정을 빼면 3건 실패
4. tsc · test-money 103/0 · test-settlement-period 57/0 · test-accounting-guard 33/0 ·
   test-due-date 33/0 · 돈 정합 0건 · build 성공
