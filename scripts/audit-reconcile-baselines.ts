...
//
// 범위 — isReconcile=true 점검만 본다. 보정 화면이 '지난 기간 사용으로 기록'으로 저장한 건은
// 일반 점검(isReconcile=false)이라 여기 안 잡히는데, 그건 누락이 아니라 의도다. 일반 점검은
// 프리필이 부풀면 그만큼 소모량으로 드러나므로(overview intervalPairs) 조용히 편입되지 않는다.
// 이 스크립트가 쫓는 것은 '차이가 소모로도 안 잡혀 오차가 장부에 영구 편입되는' 경우뿐이다.
//
