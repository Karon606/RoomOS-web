...
// 함께 움직여야 하는 한 쌍이고, 한쪽만 바뀌면 종이 한 장에 이름이 둘이 된다.
{
  ...
  //
  // **이 표를 DB 사전까지 훑도록 넓히지 마라.** avoid 는 부분일치라, 살아있는 멀쩡한 줄 둘이
  // 거짓 양성으로 붉게 선다 — ja `短期入室料金表` 가 `入室料` 를, zht `短期入住費率表` 가
  // `入住費` 를 품는다. 둘 다 **요금표의 이름**이지 요금의 이름이 아니라 옳은 문장이다
  // (번역가 패널 2026-09-11). 넓히려면 avoid 를 앵커 문구로 바꾸고 그 예외를 함께 세워야 한다.
  // 지금은 코드 상수만 보므로 이 함정에 안 걸린다.
...
// 어긋난다 — 본문 18건과 한 덩어리로 가야 해서 운영자가 원어민 확인 뒤로 미뤘다.
{
  ...
  const DOC_NAME: Record<string, string> = {
    ...
    ja: '入居契約書',
    zh: '入住合同',   // 본토 표준은 '书' 를 안 붙인다(劳动合同·房屋租赁合同). 대만은 반대다.
    zht: '入住契約書',
  }
  eq('문서 이름이 종이 제목과 같다',
