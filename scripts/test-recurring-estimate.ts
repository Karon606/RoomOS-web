...
// ── 우선순위: 예약금액 > (비변동)계약액 > 작년 같은 달 > 최근 3개월 평균 > 기본액 ──
eq('예약금액이 모든 이력을 이긴다',
  effectiveRecurringAmount(src({ pendingAmount: 1_911_220, priorYearActual: 800_000, recentAvg: 833_323 })), 1_911_220)
eq('예약금액 라벨',
    ...
    for (const priorYearActual of [null, 22_000]) {
      for (const recentAvg of [null, 33_000]) {
        for (const amount of [0, 44_000]) {
          const s = src({ pendingAmount, isVariable, priorYearActual, recentAvg, amount })
          const { amount: got, basis } = recurringEstimate(s)
          if (basisAmount[basis](s) !== got) {
            fails.push(`라벨-금액 불일치: basis=${basis} 금액=${got} (${JSON.stringify({ pendingAmount, isVariable, priorYearActual, recentAvg, amount })})`)
          } else combos++
        }
      }
    }
...
eq('전 조합에서 채택 근거와 금액이 같은 항', combos, 32)
