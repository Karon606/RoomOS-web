...
{
  const lease = { rentAmount: OLD, room: ROOM }
  ...
  const mid = { rentAmount: OLD, room: { scheduledRent: NEW, rentUpdateDate: new Date(2026, 6, 15) } }
  ...
  const flat = { rentAmount: OLD, scheduledRent: NEW, rentUpdateMonth: UPD }
  ...
  eq('경계: 평탄화가 room 보다 우선', billForLeaseMonth(
    { rentAmount: OLD, scheduledRent: 500_000, rentUpdateMonth: UPD, room: ROOM }, '2026-07', null), 500_000)
}
...
type RoomSched = { scheduledRent: number | null; rentUpdateDate: Date | null }
type LeaseLite = { rentAmount: number; isShortTerm?: boolean; checkoutProratedMonth?: string | null; discounts?: null }
...
function rewriteLocksForSchedule(lease: LeaseLite, recs: Rec[], prev: RoomSched, next: RoomSched) {
  ...
  for (const mon of months) {
    ...
    const base = { rentAmount: lease.rentAmount, discounts: lease.discounts ?? null }
...

{
  ...
  // 인상 예약을 새로 걸면 — 선납된 7월 락(구가)만 인상가로 되쓰이고 6월은 그대로다.
  {
    ...
    const changed = rewriteLocksForSchedule({ rentAmount: OLD }, recs, NONE, SCHED)
  ...
  // 퇴실 일할이 걸린 달은 불변 — 그 달은 정산액이 청구 권위다.
  {
    ...
    const changed = rewriteLocksForSchedule({ rentAmount: OLD, checkoutProratedMonth: '2026-07' }, recs, NONE, SCHED)
  ...
  {
    const recs: Rec[] = [{ id: 'r7', targetMonth: '2026-07', expectedAmount: OLD }]
    eq('되쓰기: 단기는 제외', rewriteLocksForSchedule({ rentAmount: OLD, isShortTerm: true }, recs, NONE, SCHED).length, 0)
  }

  // 예약 취소 — 되쓰기가 반대 방향으로 돌아 인상분 미수가 사라진다(적용취소 정합).
  {
    const recs: Rec[] = [{ id: 'r7', targetMonth: '2026-07', expectedAmount: NEW }]
    const changed = rewriteLocksForSchedule({ rentAmount: OLD }, recs, SCHED, NONE)
...
// 예약 필드는 비워진다(tenants/actions.ts ~776). 새 계약 rentAmount 는 그 baseRent 를 물려받는다.
{
  ...
  const afterApply = { rentAmount: NEW, room: { scheduledRent: null, rentUpdateDate: null } }
  ...
  const keepSched = { rentAmount: OLD, room: ROOM }
  ...
  eq('재입실: 엔진은 상태를 모른다(호출부 필터 책임)', billForLeaseMonth(keepSched, '2026-07', null), NEW)
}
...
// 그런데 billForLeaseMonth 의 effectiveBaseRent 는 거주 축 하나만 읽고 lease 상태를 분기하지 않는다.
{
  ...
  const activeLease = { rentAmount: OLD, room: ROOM }        // ACTIVE 실거주자
  const nonResLease = { rentAmount: NR, room: ROOM }         // NON_RESIDENT 명의자
  ...
  // 결함 — 적용월부터 비거주 계약이 거주 인상가로 청구된다(100,000 이 480,000 으로 4.8배).
  knownFail(
    '418호: NON_RESIDENT 계약 7월도 비거주 요율이어야 한다',
    billForLeaseMonth(nonResLease, '2026-07', null), NR, NEW,
    'effectiveBaseRent(lib/billing.ts:30)가 lease 상태를 분기하지 않고 room.scheduledRent(거주 축)를 그대로 쓴다. '
    + 'unpaid.ts·dashboard 가 NON_RESIDENT 를 청구 대상으로 조회하므로 미납 화면까지 그대로 전파된다.',
  )
  knownFail(
    '418호: NON_RESIDENT 계약 8월 이후도 비거주 요율',
    billForLeaseMonth(nonResLease, '2026-08', null), NR, NEW,
    '같은 원인 — 적용월 이후 전 구간이 오염된다.',
  )
  ...
  const nrOnly = { rentAmount: NR, room: { scheduledRent: null, rentUpdateDate: null } }
  knownFail(
    '사무실: 비거주 예약 인상(120,000)이 7월부터 반영돼야 한다',
    billForLeaseMonth(nrOnly, '2026-07', null), 120_000, NR,
    'BillingLeaseFields.room 에 nonResidentScheduled/nonResidentRentDate 를 받는 칸이 없다. '
    + '적용일(applyScheduledRents)이 지나 nonResidentRent 로 옮겨진 뒤에야 반영된다 — 선납은 구가로 처리된다.',
  )
  ...
  // 표시만 틀리는 것이 아니라 DB 의 expectedAmount 가 실제로 덮어써진다.
  {
    ...
    knownFail(
      '418호: 거주 인상 예약이 NON_RESIDENT 락을 되쓰면 안 된다',
      [changed.length, recs[0].expectedAmount], [0, NR], [1, NEW],
      '되쓰기 정본이 상태를 분기하지 않아 비거주 계약의 저장된 청구액이 거주 인상가로 갱신된다(쓰기 경로 오염).',
    )
  }
...
{
  // 취소(둘 다 null) — 전 구간 구가.
  const cancelled = { rentAmount: OLD, room: { scheduledRent: null, rentUpdateDate: null } }
  ...
  eq('취소: room 자체가 없어도 구가(회귀 0)', billForLeaseMonth({ rentAmount: OLD }, '2026-07', null), OLD)
  eq('취소: room null 이어도 구가', billForLeaseMonth({ rentAmount: OLD, room: null }, '2026-07', null), OLD)
  ...
  eq('고아: 적용일 없으면 인상 미적용', billForLeaseMonth(
    { rentAmount: OLD, room: { scheduledRent: NEW, rentUpdateDate: null } }, '2027-01', null), OLD)
  eq('고아: 금액 없이 적용일만 있으면 구가', billForLeaseMonth(
    { rentAmount: OLD, room: { scheduledRent: null, rentUpdateDate: new Date(2026, 6, 1) } }, '2026-07', null), OLD)
  eq('고아: 예약 금액 0 은 미적용', billForLeaseMonth(
    { rentAmount: OLD, room: { scheduledRent: 0, rentUpdateDate: new Date(2026, 6, 1) } }, '2026-07', null), OLD)
  eq('고아: 잘못된 적용일은 미적용', billForLeaseMonth(
    { rentAmount: OLD, room: { scheduledRent: NEW, rentUpdateDate: '2026-13-99' } }, '2026-07', null), OLD)

  // 금액 변경 — 마지막 예약값이 이긴다.
  eq('변경: 인상가를 500,000 으로 고치면 그 값', billForLeaseMonth(
    { rentAmount: OLD, room: { scheduledRent: 500_000, rentUpdateDate: new Date(2026, 6, 1) } }, '2026-07', null), 500_000)
  // 적용월 변경(7월 → 8월) — 7월은 구가로 되돌아간다.
  const moved = { rentAmount: OLD, room: { scheduledRent: NEW, rentUpdateDate: new Date(2026, 7, 1) } }
  ...
  eq('변경: 인하 예약도 적용월부터', billForLeaseMonth(
    { rentAmount: OLD, room: { scheduledRent: 400_000, rentUpdateDate: new Date(2026, 6, 1) } }, '2026-07', null), 400_000)
  ...
  eq('적용 완료: rentAmount 가 인상가, 예약 비움', billForLeaseMonth(
    { rentAmount: NEW, room: { scheduledRent: null, rentUpdateDate: null } }, '2026-08', null), NEW)
  // 적용 완료 후에도 과거 달은 락인이 지킨다(소급 재계산 금지).
  eq('적용 완료: 과거 달은 락인이 구가를 지킨다', billForLeaseMonth(
    { rentAmount: NEW, room: { scheduledRent: null, rentUpdateDate: null } }, '2026-05', OLD), OLD)
}
