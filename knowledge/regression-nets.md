...
**축 3 — radius (2026-09-17, 독립 검수 지적 B3).** 배경·보더만 보던 그물이 `rounded-lg`(10px)로
선 칸을 통과시켰다. §12 는 **전 입력 6px** 이다. 어긴 칸의 주석은 "이 카드의 버튼들과 같은 한 벌"
이라 적고 있었는데 **맞출 대상이 틀렸다 — 입력은 버튼이 아니라 제 폼의 입력 형제에 맞춘다.**

- 전수 95곳 — 6px 90 · 위반 4 · 인라인 편집 예외 1. 90 대 4 라 다수파가 아니라 규칙이다.
- **호출부에 직접 적힌 글자만 본다.** `resolve()` 로 펼친 상수 뭉치에는 그 파일의 남남인 선언까지
  딸려 와 `rounded-lg` 가 섞인다 — 축을 세울 때 실제로 9곳이 오탐이었다.
- 축을 세운 회차는 제 검수 범위(미푸시 커밋 셋) 밖인 셋을 `RADIUS_KNOWN` 에 적어 두고 안 고쳤다 —
  남의 화면을 말없이 갈면 다음 검수가 원인을 못 찾는다는 판단이었다. **같은 날 코디네이터 판정으로
  셋을 함께 6px 로 닫고 명단을 비웠다.** 그물은 이제 95곳 전수이고, 찍던 자리도 지웠다(찍을 것이
  없다). **명단을 다시 채우지 말 것** — 한 줄을 더하는 것은 빚을 갚는 게 아니라 늘리는 것이다.
- 닫은 셋. `TenantClient:3193` 과 `DueDayTempAdjustWidget:224` 는 같은 UI 지만 **한 자리가 아니다** —
  위젯을 부르는 곳은 `PaymentBody` 하나뿐이고 `TenantClient` 는 임포트도 안 하는 붙박이 사본이라
  껍데기가 갈린다(붙박이 `--warm-border`·`px-2.5`, 컴포넌트 `--warning-ring`·`px-3`). 따로 고쳤다.
  `InventoryClient:3448` 은 **같은 행**의 시각 input 과 높이가 38px 로 이미 같은데 모서리만
  14px 대 6px 이라, 한 줄 안에서 알약 옆에 각진 칸이 서 있었다(402px 실측).
...
## 스침 가드와 호실 열 (2026-08-24, 오류신고 16f691e1)

그물 둘. `scripts/test-tap-guard.ts`(순수 판정 31축)와
`scripts/check-calendar-room-open.mjs`(소스 가드), 둘 다 `verify:fast` 등재.

**순수 함수로 뽑은 이유**가 이 자리의 핵심이다. 제스처 판정은 화면 안에 두면 회귀로 못 재고,
못 재는 규칙은 다음 사람이 조용히 바꾼다. `lib/tapGuard` 가 판정만 지고 React 배선은 화면이 진다.
- **키보드 축이 이 그물의 존재 이유다.** 이동 래치가 키보드 활성화를 삼키기 시작해도 화면에는
  아무 표시가 없다 — 마우스를 못 쓰는 사용자만 앱을 못 쓰게 되는, 눈으로 절대 안 보이는 회귀다.
- **시간 인자가 등장하면 그것이 곧 회귀다**(운영자 지시 — 저속 탭 오탐). 파일 머리에 적어 뒀다.
- 역주입 4종 통과 — 빗변을 축별 abs 로 되돌림 · 키보드 예외 제거 · 흐름 축 제거 · 임계 16→10.

**소스 가드가 필요한 이유**: 호실 열이 `<button>` 에서 `<div>` 로 되돌아가도 **화면 표시가 한
픽셀도 안 바뀐다.** 눈으로는 못 잡는 회귀라 태그와 접근명 자체를 위반으로 잡는다. 축 다섯 —
① button + onClick + type ② 접근명에 `fmtRoomNo`(WCAG 2.5.3, 빼면 음성 제어로 부를 이름이
사라진다) ③ 스페이서 둘은 비인터랙티브 유지 ④ 엘리먼트에 Tailwind outline 유틸 금지
(언레이어드 globals.css 가 유틸을 통째로 지운다) ⑤ 스크롤러의 캡처 가드 생존.

**반쪽이 되기 쉬운 자리 둘, 역주입으로 겪은 뒤 조였다.**
1. `<(\w+)\b([^>]*?)>` 로는 태그를 못 자른다. 속성값에 화살표 함수가 있어 `>` 가 속성 안에 산다
   (`onClick={() => onOpenRoom(...)}`). 중괄호 깊이와 따옴표를 세며 진짜 닫는 `>` 를 찾는다.
   조이기 전에는 **위반이 아닌 것이 위반으로** 잡혔다.
2. `includes('onClickCapture')` 는 `onClickCaptureXX=` 를 통과시킨다(역주입 4에서 실제로 통과했다).
   `=` 까지 요구해야 이름만 바꾼 무력화가 잡힌다.
역주입 8종 통과 — div 되돌림 · 접근명에서 번호 제거 · 스페이서 버튼화 · 핸들러 이름 변조 ·
outline 유틸 덧붙임 · `suppressesTap` 무력화 · coarse 최소 높이 규칙 제거 · 선택자 넓힘.
...
### 2026-09-17 독립 검수 뒤 — 축 셋을 더했다

- **ⓔ 순서.** 인라인 확인 줄은 `[확인][취소]` 다(전수 7 대 0). **정본(`PaymentRecordList`)도 감시
  대상에 넣었다** — 기준이 뒤집히면 나머지가 그쪽으로 끌려간다. 폼 박스·모달 푸터는 §13·§14 의
  반대 축이라 빼야 하는데, 가르는 표식은 `justify-end` 다. 이 표식 없이 창만 좁혔더니
  수납 편집 폼(취소 좌)이 걸려 나왔고 **그것은 결함이 아니라 다른 축의 올바른 줄**이었다.
- **ⓓ 어휘 확장.** 리터럴 `처리일` 만 보던 축은 "정본의 기본 사유를 그대로 돌려주는" 클래스를
  한 건도 못 잡았다(자재 여섯이 `완료일이 미래입니다` 를 점검·입수·폐기 폼에서 띄우고 있었다).
  이제 `GUARD_SITES` 가 가드 자리마다 그 화면의 낱말을 적고, **명단에 없는 새 가드는 fail closed** 다.
  열쇠는 줄 번호가 아니라 **감싸는 함수 이름**이라 코드가 밀려도 안 썩는다.
- **ⓕ 발급일.** 서류 발급일 칸 둘(정본 `DatePicker` 의 `maxDate` · 툴바 칩의 네이티브 `max`)과
  두 발급 라우트의 서버 가드.
- **ⓒ 를 개수로 바꿨다.** 종전에는 "파일 어딘가에 `maxDate` 가 있으면 통과"라, 칸이 둘인 화면에서
  하나를 지워도 초록불이 떴다. **역주입이 처음으로 안 걸린 자리**이고, 그래서 `dateFields` 로
  개수를 박았다. 그물을 고친 뒤 같은 주입이 빨개졌다.
- 2차 역주입 6종 전부 빨강. `const` 만 남기고 찍는 자리 지우기는 ⓒ 개수 축이, import 만 남기고
  호출 지우기는 `GUARD_SITES` 의 '사라졌다' 검사와 eslint `no-unused-vars` 가 각각 잡는다.
