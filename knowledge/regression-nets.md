...
- 이번 검수 범위(미푸시 커밋 셋) 밖인 셋은 `RADIUS_KNOWN` 에 적었다. 고치는 일 자체는 낱말
  하나씩이지만 남의 화면을 말없이 갈아 두면 다음 검수가 원인을 못 찾는다. **매 실행마다 소리 내어
  찍는다** — 조용한 명단은 곧 잊히는 빚이다. 운영자 승인이 나면 셋을 고치고 명단을 지운다
  (`TenantClient:3193`·`DueDayTempAdjustWidget:224` 는 같은 위젯의 두 벌,
  `InventoryClient:3448` 은 같은 행의 시각 input 이 `rounded-sm` 이라 한 줄 안에서 두 벌이다).
