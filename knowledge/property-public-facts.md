...
- 4MB 상한. 도장·로고(5MB)보다 낮은 이유는 이 파일만 서버리스 함수가 바이트를 통째로 실어
  응답하기 때문이다(그 경로 실질 한도 4.5MB). **이 상한은 변환 뒤 크기 기준이다**(아래).
- 형식을 바꾸지 않고 올린 그대로 보낸다. 서류 3종과 달리 PNG 변환 선택지를 주지 않는다 —
  등록증은 우리가 그린 서류가 아니라 받아 둔 원본이다.
- **저장 형식은 PDF 하나다**(2026-09-16 운영자 결정). 사진을 올려도 되지만 그 사진은 **올라가기
  전에 브라우저가 PDF 한 장으로 바꾼다**(`lib/uploadImage.fileToUploadPdf`). 아이폰이 찍는 HEIC 를
  그대로 저장하면 상담 문자·메일 첨부가 **열리지 않는 파일로 나가고**, 서버에는 그것을 디코드할
  수단이 없다(sharp 의 libheif 가 AVIF 만 문다, 실측). 서버가 받는 형식도 `application/pdf` 로
  좁혔다 — JPEG·PNG 는 낡은 화면이 남아 있을 때의 안전망이고 **HEIC/HEIF 는 거절된다.**
  변환은 EXIF 회전을 픽셀에 박고, 용지 비율은 사진 비율 그대로다(위아래 흰 띠를 만들지 않는다).
  "형식을 바꾸지 않는다" 는 위 줄은 **저장한 뒤 내보낼 때**의 규칙이다([[doc-file-format]]).
- **입주자 서류 시트도 이 값을 쓴다**(2026-09-16). 계약 그룹 다음에 '영업장 서류' 한 행으로 서고,
  미등록 영업장에는 그 그룹이 아예 없다. 소비처가 셋이 됐으므로(환경설정 미리보기·상담 도구·
  서류 시트) 이 두 칼럼을 고칠 때 셋을 함께 본다. 서류 시트 쪽은 `/api/biz-cert` 가 아니라
  `/api/doc-file`·`/doc/[fileId]` 를 타는데, 그 둘의 소유 검증에 **같은 날 함께** 네 번째 가지
  (`property.findFirst({ id: propertyId, bizCertDriveFileId })`)를 넣었다. 임의 Drive ID 를 끼울
