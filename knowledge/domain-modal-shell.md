...
1. `bandHeight(height, lastGood, allowShrink)` 하나가 **패널 높이에 쓸 띠 높이**를 정한다.
   거부는 `0` 이 아니라 `lastGood` 으로 답한다 — 한 프레임 값이 조금 낡는 것이 레이아웃이
   통째로 흔들리는 것보다 낫다.

   **처음에는 인셋도 이 한 값에서 나오게 했다가 하루 만에 반대쪽이 터졌다(2026-09-17).**
   관문이 두 겹인데 쓰는 곳이 다르다는 것을 그때 알았다.
   · **위생 검사**(불가능값을 버리고 직전 유효값을 쓴다, `usableVvHeight`) — 높이와 인셋 **둘 다**.
     찢어진 스냅샷을 막는 문이 이것이다.
   · **방향 관문**(줄이는 것은 resize 에서만, `shouldWriteVvHeight`) — **패널 높이만**.
     인셋에 걸면 키보드가 서서 띠가 진짜 줄어든 팬 프레임에서 관문이 작아진 값을 거부하고,
     인셋까지 '키보드 없음'이라 답해 패널이 키보드 밑까지 뻗는다. 운영자 신고 원문 —
     "키보드가 가려져서 숫자 입력이 힘들다".
   전수 훑기 실측 — 띠가 416 일 때 패널이 **384 에서 780** 으로 커졌다. 고친 뒤 33개 조합에서
   가림이 사라지고 **새로 무너지는 조합은 0건**이다(시공 전후 차이 57건이 전부 커지는 방향이라
   이 변경이 패널을 무너뜨리는 일은 애초에 불가능했다).
2. `overlayInsets` 의 두 항 모두 `[0, innerHeight - height]` 안으로 죈다. 주석이 선언한
   불변식(`top + bottom = innerHeight - height`)을 **양쪽 끝에서 다** 강제한다.

**교훈 둘. 보호가 한쪽에만 걸린 값 쌍은 언젠가 갈린다.** 그리고 **그렇다고 한 값으로 묶으면
반대쪽이 터진다** — 두 값이 뜻이 다르면 관문도 뜻에 맞게 갈라야 한다(2026-09-17 회귀).

앞의 것부터 적는다.  2026-08-29 에 `top` 만 잠갔고,
