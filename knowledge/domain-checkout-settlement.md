...
- 환불 미처리(스냅샷 없이 청구 확정만 있고 받은 돈이 더 많음, `getPendingRentRefundNotice`).
  [환불 기록]이 `finalizeRentRefund` 한 번. 재확정 둘째가 막힌 중간 상태도 같은 모양이라 입구가 하나.
...
가 비어 카드가 사라진다(재확정 입구 없음). '환불 미처리' 0원 출구("돌려주지 않았다"의 기록)는
`finalizeRentRefund` 가 환불 0 을 거부해 미구현 — 운영자 결정 항목.
