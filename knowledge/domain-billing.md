...
## 입주달 첫 달 규칙 — 납부일과 입주일이 다르면 첫 달에 일할 합산 (2026-09-06 운영자 확정, 해석 1)

`lib/billing.ts firstMonthGap`. 납부일이 입주일과 다른 계약의 **입주한 달** 청구가 이렇게 선다.
- 납부일=입주일의 날(파생 등가, `sameDueDay`)이면 무발동 — 기존 계약 전부가 이 갈래라 무회귀.
- 다음 납부일이 입주달 안(P>D)이면 **만월 + 앞구간(입주일부터 납부일 전날) 일할 가산**.
- 다음 납부일이 다음 달(P<D)이면 **다음 달 납부일까지의 일할만** (그 달 만월 없음).
- 단가는 월/30 floor, 가산 일수 상한 30(구조상 도달 불가, 방어선). 락인·단기 우선.
- 입주달 첫 청구의 **기한은 입주일**(`firstMonthDueYmd`) — 홈·미수 표시는
  `effectiveDueDayForMonth`(dashboard/unpaid 복제 두 곳)가 오버라이드 → 입주달 → dueDay 순.
- 퇴실월 무청구 예외(`isCheckoutNoBillingMonthFor`)는 입주달 발동 시 양보한다(입주달=퇴실월인
  초단기 거주가 0원이 되는 구멍 방지).
- 정산 구간(`settlementPeriodFor`)은 입주월 보정을 항상 하고 귀속월도 입주달이다.
- `BillingLeaseFields.dueDay` 는 **필수 선언** — select 누락을 tsc 가 잡는다(소비처 9파일 배선).
- 배포 안전 증명: 실DB 옛/새 엔진 대조(check-first-month-shift, verify:db) 노출 창 0건.
