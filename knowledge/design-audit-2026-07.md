...
- **토스트 다크모드 대비(2026-07-10 신고)**: SaveFeedback info 토스트가 `var(--ink)` 배경 + 크림 하드코딩 글자 → 다크에서 --ink가 --d-ink(밝음)로 뒤집혀 크림-온-크림. 수정: 토스트 칩은 모드 불변 어두운 서피스로 통일(info=#3D2418 고정). 규칙: 모드 불변이어야 하는 오버레이(토스트 등)에는 뒤집히는 시맨틱 토큰(--ink 등)을 배경으로 쓰지 말 것 — 가이드 599행 '다크 전환은 토큰 치환만' 참조.
