...
- [[domain-completion-date]] — **완료 처리 날짜는 운영자가 적는다**(2026-09-17): 값 결정 정본(lib/completionDate 고른 날>기존 값>지금·칸 타입 둘)·사실 자리와 로그 자리 구별·전수 표·인라인 확인 줄 문법(라벨 '완료일'·버튼 '완료 기록'·'처리일' 금지)·적용취소 후 재완료 날짜 복원(prev 스냅샷)·미래는 화면 maxDate 와 서버 두 겹·넷째 시간 감지망(스키마에서 칸 이름을 뽑아 fail closed)
- [[design-visual-identity]] — 'AI 티' 시각 판정 원칙: 원은 도형-기능만·글자면은 각·립은 예외 전용(수납 관리 예외)·코랄 채움은 CTA만, 존치 목록과 클로드 디자인 인계 경계, 2차 정비(배지 §04 트라이어드·ring 폐지·홈 알림 립·간격 다수파·읽기전용 박스 높이·그라데이션 그물), 다크 코랄 글자 후보 목록
- [[double-submit-guard]] — 중복 제출 방어(대기 표시 정본·서버 가드 문법·전수 감사 결과·화면 잠금 단일 의존 자리)
- [[sign-guide-language]] — 원격 서명 안내 8언어: 스냅샷 박제·국적 기본값·병기 규칙
- [[domain-sign-documents]] — 추가 서류(제3 서류): 저장 자리 셋·key 불변·삭제 없음·스냅샷 축
- [[domain-contracts]] — 계약서/동의서 서명·환불조항·주소·인쇄 · **계약서 손댈 때 체크리스트(A~F)** · 서명 진행 판정(signStage, 반쪽은 양방향) · 서명란 성명은 서버 단일 변수 · 남은 서명 안내(무텍스트 신호) · 납부일 게이트(문 넷·원천 저장)
- [[contract-translation-print]] — 참고용 번역본 검수 PDF: 언어별 글꼴 실측(Pretendard 는 한자·벵골 0자)·왜 fonts-i18n 을 따로 두나·글꼴 실패에 부분 성공 없음(운영자가 두부를 못 읽는다)·저장 안 하는 종이라 적용취소 없음·감지망 축 0~6
- [[sign-evidence-axes]] — 서명 증거의 두 축(링크 행 · 계약 이미지). 화면마다 다른 것을 읽고 있다(실측 37건 중 15건 갈림)
...
- [[doc-issue-history]] — 발급 서류 화면 넷의 소관 분리(시트=보내기·계약서 패널=판본·전역=검색삭제·이력 목록=열람뿐)·'이번 달 확인서 작성' 문은 stale×납부 곱·stale 판정은 귀속월(선납 중복 봉합)
- [[tenant-own-phone-fallback]] — 본인 연락처와 대체: 문자는 비상으로 돌리고 종이는 비워 둔다 · 예약 확정 게이트(메신저만은 거부·유선과 해외는 통과·소급 안 막음) · 폼 안내는 입력값 축
- [[doc-name-style]] — 서류에 찍을 성명 표기(한글·영문·현지) 정본: 기본값 서열 다섯(저장값>앞 서류>사람 단위 Tenant.docNameStyle>외국인이면 영문>한글)·소급 없음·카드 표시와 별개 축·외국인 판정 둘(빈 국적의 답이 반대)·영문 이름은 외국인등록증 표기
...
- [[cash-receipt-refund]] — 현금영수증은 앱 표시일 뿐 실제 발행은 홈택스(임형진 사례)·환불 재기록의 증빙 메타 유실·**현금영수증 합계는 발행일 축, 카드는 입금일 축**(2026-08-24 정정)·미반환 보증금 수익은 정산일 축(기본 퇴실일)·기한 알림 세 자리(임박·자진발급 감경 창·요약, 조회창은 인수 컷오프)·**탭 화면 문법**(**머리가 센 목록이 머리 바로 밑에 온다** — 발행 내역이 첫 블록, 2026-09-22 운영자 재지적으로 순서 뒤집음·`발행 내역`이지 `이력` 아님·행이 구성을 불리언으로만 말한다, 신고 249f98cc)·**보증금·청소비는 받을 때 발행 대상 아님**(2026-09-21 운영자 확정, 2026-09-22 시공 완료)
...
- [[domain-checkout-revert]] — 퇴실·투어 완료·입실 처리 적용취소: 목적지는 예정일, 차단 셋(반환 기록·환불 스냅샷·이중 점유), 뒷문 게이트 (2026-09-10)
