...
## 오류신고 확인 규칙
세션 시작 시 `node --env-file=.env.local scripts/check-error-reports.mjs` 를 실행해 미처리(open) 오류신고가 있는지 확인한다.
- 있으면 운영자에게 내용을 보고하고 "진행할까요?"를 묻는다. 승인하면 작업, 아니면 운영자가 보강/수정 후 진행.
